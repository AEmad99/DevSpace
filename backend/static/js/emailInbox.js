...
let _loading = false;
let _expanded = false;
let _docResolver = null;  // () => Promise<documentModule|null> — set in init(), called per-use
let _docCache = null;      // resolves the resolved module so we don't re-await after first load
...
// Returns the resolved document module (loading it lazily on first call),
// or null if it isn't loaded yet and the resolver returned null. Callers
// null-check before using — every usage is gated by a user action (clicking
// an email, opening a draft) so the first call naturally waits on the
// module fetch.
async function _docModule() {
  if (_docCache) return _docCache;
  if (!_docResolver) return null;
  _docCache = await _docResolver();
  return _docCache;
}

// Build a Proxy that looks like the document module but defers loading until
// a property is actually accessed. Each method returns a Promise that resolves
// to the real method's result, so existing call sites
// (`documentModule.openPanel()`, `documentModule.loadDocument(id)`, …) keep
// working without modification — they just need to `await` the call.
function _makeLazyDocProxy(resolver) {
  if (!resolver) return null;
  return new Proxy({}, {
    get(_t, prop) {
      return (...args) => _docModule().then(mod => {
...
export function init(docResolver) {
  _docResolver = docResolver || null;
  _bindEvents();
  // Init the library popup with a callback to open emails. The library
  // also gets the lazy resolver so it can open email drafts without forcing
  // document.js to load on the email library's own init path.
  initEmailLibrary({
    documentResolver: docResolver,
    // Legacy alias — emailLibrary expects a `documentModule` argument; pass
    // a Proxy that resolves the real module on first property access so the
    // existing call sites (`documentModule.openPanel()` etc.) keep working
    // without modification.
    documentModule: _makeLazyDocProxy(docResolver),
...
function _bindEvents() {
  ...
  _refreshUnreadCount();
  window._dsTrackInterval(setInterval(_refreshUnreadCount, 60000));
...
async function _openEmail(em, itemEl, preloadedData = null, mode = 'reply', noteHint = '', prefilledBody = '') {
  ...
  try {
    ...
    if (docMod) {
      ...
      const existingDocId = (reuseExisting && docMod.findEmailDocId)
        ? docMod.findEmailDocId(em.uid, _currentFolder)
        : null;
      if (existingDocId) {
        if (!docMod.isPanelOpen()) docMod.openPanel();
        await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
        await docMod.loadDocument(existingDocId);
      } else {
        ...
        if (doc.id) {
          const wasOpen = docMod.isPanelOpen();
          if (!wasOpen) docMod.openPanel();
          await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
          ...
          if (docMod.injectFreshDoc) {
            ...
            await docMod.loadDocument(doc.id);
          }
        }
      }
    }
