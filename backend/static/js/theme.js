...
export const THEMES = {
  dark:       { bg:'#282c34', fg:'#9cdef2', panel:'#111111', border:'#355a66', red:'#61afef' },
...
const DEFAULT_CHAT_MAX = 1000;
const CHAT_MAX_OPTIONS = [800, 900, 1000, 1100, 1200, 1400];
const DEFAULT_MSG_FONT_SCALE = 1;
const MSG_FONT_SCALE_OPTIONS = [0.85, 1, 1.15, 1.3];
...
function deriveSyntaxColors(colors) {
  ...
  const [redH, redS, redL] = hexToHSL(colors.red || '#61afef');
  const isDark = bgL < 50;
...
const ADV_KEYS = [
  ...
  { key: 'brandColor',         css: '--brand-color',       label: 'DevSpace Logo',    group: 'Sidebar' },
...
export function applyColors(colors) {
  ...
  _updateFavicon(colors.red || '#61afef');
}
...
function _updateFavicon(fg) {
  ...
  if (routeShape) {
    ...
    svg = `<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><path d='M11 7L5 16L11 25' fill='none' stroke='${fg}' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'/><path d='M21 7L27 16L21 25' fill='none' stroke='${fg}' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'/><path d='M16 11.5Q16 16 20.5 16Q16 16 16 20.5Q16 16 11.5 16Q16 16 16 11.5Z' fill='${fg}'/></svg>`;
  }
...
export function applyChatMax(px) {
  const n = Number(px);
  const safe = Number.isFinite(n) && n > 0 ? n : DEFAULT_CHAT_MAX;
  document.documentElement.style.setProperty('--chat-max', safe + 'px');
}

export function applyMsgFontScale(s) {
  const n = Number(s);
  const safe = Number.isFinite(n) && n > 0 ? n : DEFAULT_MSG_FONT_SCALE;
  document.documentElement.style.setProperty('--chat-msg-font-scale', String(safe));
}
...
export function save(name, colors, opts) {
  ...
  if (opts) {
    ...
    if (opts.chatMax && opts.chatMax !== DEFAULT_CHAT_MAX) obj.chatMax = opts.chatMax;
    if (opts.msgFontScale && opts.msgFontScale !== DEFAULT_MSG_FONT_SCALE) obj.msgFontScale = opts.msgFontScale;
...
export function initThemeUI() {
  ...
  function _getOpts() {
    ...
    const cms = document.getElementById('theme-chat-max-select');
    const mfs = document.getElementById('theme-msg-font-scale-select');
    ...
    if (cms) opts.chatMax = parseInt(cms.value, 10);
    if (mfs) opts.msgFontScale = parseFloat(mfs.value);
  ...
  allGrids.forEach(g => {
    g.querySelectorAll('.theme-swatch').forEach(sw => {
      sw.addEventListener('click', (e) => {
        ...
        const cm = (ct && ct.chatMax) || (saved && saved.chatMax) || DEFAULT_CHAT_MAX;
        const msc = (ct && ct.msgFontScale) || (saved && saved.msgFontScale) || DEFAULT_MSG_FONT_SCALE;
        applyFontDensity(f, d);
        applyChatMax(cm);
        applyMsgFontScale(msc);
        ...
        if (fs) fs.value = f;
        if (ds) ds.value = d;
        if (cms) cms.value = String(cm);
        if (mfs) mfs.value = String(msc);
        if (ps) ps.value = p;
        ...
        save(name, colors, { font: f, density: d, chatMax: cm, msgFontScale: msc, bgPattern: p, bgEffectColor: ec, bgEffectIntensity: ei, bgEffectSize: sz, frosted: fr });
      });
    });
  ...
  if (resetBtn) {
    ...
    newReset.addEventListener('click', () => {
      ...
      applyChatMax(DEFAULT_CHAT_MAX);
      applyMsgFontScale(DEFAULT_MSG_FONT_SCALE);
      applyBgPattern('none');
      const fs = document.getElementById('theme-font-select');
      const ds = document.getElementById('theme-density-select');
      const cms = document.getElementById('theme-chat-max-select');
      ...
      if (cms) cms.value = String(DEFAULT_CHAT_MAX);
      if (mfs) mfs.value = String(DEFAULT_MSG_FONT_SCALE);
      if (ps) ps.value = 'none';
      grid.querySelectorAll('.theme-swatch').forEach(s => s.classList.remove('active'));
  ...
  const _initChatMax = (saved && saved.chatMax) || DEFAULT_CHAT_MAX;
  const _initMsgFontScale = (saved && saved.msgFontScale) || DEFAULT_MSG_FONT_SCALE;
  ...
  applyChatMax(_initChatMax);
  applyMsgFontScale(_initMsgFontScale);
  ...
  const chatMaxSelect = document.getElementById('theme-chat-max-select');
  const msgFontScaleSelect = document.getElementById('theme-msg-font-scale-select');
  ...
  if (chatMaxSelect) {
    const ncm = chatMaxSelect.cloneNode(true); chatMaxSelect.parentNode.replaceChild(ncm, chatMaxSelect);
    ncm.value = String(_initChatMax);
    ncm.addEventListener('change', () => {
      applyChatMax(parseInt(ncm.value, 10));
      const s = getSaved(); if (s) _saveFull(s.name, s.colors);
    });
  }
  if (msgFontScaleSelect) {
    const nmfs = msgFontScaleSelect.cloneNode(true); msgFontScaleSelect.parentNode.replaceChild(nmfs, msgFontScaleSelect);
    nmfs.value = String(_initMsgFontScale);
    nmfs.addEventListener('change', () => {
      applyMsgFontScale(parseFloat(nmfs.value));
      const s = getSaved(); if (s) _saveFull(s.name, s.colors);
    });
  }
  if (patternSelect) {
  ...
  if (harmonyAccentEl && _harmonyHex) {
    _harmonyHex.textContent = harmonyAccentEl.value || '#61afef';
