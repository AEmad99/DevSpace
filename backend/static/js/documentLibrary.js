...
// ---- Library animation helpers ----
  ...
  function _showLibDropdown(anchor, items, opts) {
    ...
    // Use a custom class instead of `session-dropdown-menu`. The global
    // click-away listener in sessions.js (`_initDropdownDismiss`) hides
    // every `.session-dropdown-menu` on outside clicks — including the
    // library dropdown we just appended to <body>, which made the
    // Chats/Documents/Research/Archive ⋮ menu items stop working.
    dd.className = 'dropdown _lib-dd';
  ...
  export function openLibrary(opts) {
    ...
    function _renderArcGrid() {
      ...
      for (const r of filtResearch) {
        ...
        card.querySelector('._arc-res-menu').addEventListener('click', (e) => { e.stopPropagation(); _showLibDropdown(e.currentTarget, [
          { label: 'Open', action: () => window.openResearchReport(r.id) },
    ...
    async function _toggleResearchPreview(card, item) {
      ...
      if (openBtn) openBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        window.openResearchReport(item.id);
      });
      ...
      grid.querySelectorAll('.doclib-research-delete').forEach(btn => {
        btn.addEventListener('click', (e) => {
          ...
          _showLibDropdown(btn, [
            { label: 'Open', action: () => window.openResearchReport(rid) },
