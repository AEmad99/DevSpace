...
if (!window._dsIntervals) window._dsIntervals = new Set();
if (!window._dsTimeouts) window._dsTimeouts = new Set();
window._dsKillAllIntervals = function() {
  for (const id of window._dsIntervals) clearInterval(id);
  window._dsIntervals.clear();
};
window._dsKillAllTimeouts = function() {
  for (const id of window._dsTimeouts) clearTimeout(id);
  window._dsTimeouts.clear();
};
window._dsTrackInterval = function(id) {
  window._dsIntervals.add(id);
  return id;
};
window._dsTrackTimeout = function(id) {
  window._dsTimeouts.add(id);
  return id;
};
...
const _AUTO_WIRE = {
  ...
  'code-workspace-modal': { rail: 'rail-code',      sidebar: 'tool-code-btn' },
...
const _scanTimer = window._dsTrackInterval(setInterval(_scanAndWire, 1000));
...
window.addEventListener('pagehide', () => { window._dsKillAllIntervals(); window._dsKillAllTimeouts(); });
window.addEventListener('beforeunload', () => { window._dsKillAllIntervals(); window._dsKillAllTimeouts(); });
