...
// Close fn for the currently-open row ⋮ menu (so opening another / clicking
// away reliably tears down the previous one, including its document listeners).
let _activeMemDropdownClose = null;
...
export function renderMemoryList() {
  ...
  filtered.forEach(memory => {
    ...
    if (!selectMode) {
      ...
      // Single, self-cleaning close for THIS menu: detaches its outside-click /
      // Esc listeners and removes the element. No per-render listener buildup.
      let _docCloser = null, _escCloser = null;
      const closeDropdown = () => {
        if (_docCloser) { document.removeEventListener('click', _docCloser, true); _docCloser = null; }
        if (_escCloser) { document.removeEventListener('keydown', _escCloser, true); _escCloser = null; }
        if (dropdown.parentNode) dropdown.remove();
        if (_activeMemDropdownClose === closeDropdown) _activeMemDropdownClose = null;
      };
      ...
      pinItem.addEventListener('click', () => { closeDropdown(); togglePin(memory.id, !memory.pinned); });

      const editItem = document.createElement('div');
      editItem.className = 'dropdown-item-compact';
      editItem.textContent = '✎ Edit';
      editItem.addEventListener('click', () => { closeDropdown(); startInlineEdit(item, memory); });

      const deleteItem = document.createElement('div');
      deleteItem.className = 'dropdown-item-compact memory-dropdown-delete';
      deleteItem.textContent = '✕ Delete';
      deleteItem.addEventListener('click', () => { closeDropdown(); deleteMemory(memory.id); });
      ...
      cancelItem.addEventListener('click', (e) => { e.stopPropagation(); closeDropdown(); });
      ...
      menuBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        e.preventDefault();
        // Toggle: tear down any open menu first (incl. this one's listeners).
        const wasOpen = !!dropdown.parentNode;
        if (_activeMemDropdownClose) _activeMemDropdownClose();
        if (wasOpen) return; // second click on the same kebab → just closed it

        // Reset any leftover gesture state. A prior touch swipe could leave the
        // (reused) element at opacity:0 / translated, which would make the menu
        // re-open INVISIBLE — i.e. "the dots show but the menu won't open".
        ...
        // Measure off-screen-invisible, then place clamped to the viewport.
        dropdown.style.visibility = 'hidden';
        dropdown.style.display = 'block';
        document.body.appendChild(dropdown);

        // Clamp fully inside the viewport on ALL sides so the menu can never
        // open off-screen (a cause of "the menu won't open" on docked / scaled
        // / multi-monitor windows).
        const m = 6;
        const btn = menuBtn.getBoundingClientRect();
        const dd = dropdown.getBoundingClientRect();
        const vw = window.innerWidth, vh = window.innerHeight;
        let left = btn.right - dd.width;          // right-align to the kebab
        let top = btn.bottom + 2;
        if (left + dd.width > vw - m) left = vw - m - dd.width;
        if (left < m) left = m;
        if (top + dd.height > vh - m) top = btn.top - dd.height - 2; // flip up
        if (top < m) top = m;
        if (top + dd.height > vh - m) {           // still too tall → scroll
          dropdown.style.maxHeight = Math.max(80, vh - m - top) + 'px';
        ...
        _activeMemDropdownClose = closeDropdown;

        // Outside-click + Esc to close. Registered on the NEXT tick so the
        // opening click doesn't immediately close it, and in the CAPTURE phase
        // so it works regardless of stopPropagation elsewhere. Torn down by
        // closeDropdown.
        _docCloser = (ev) => {
          if (dropdown.contains(ev.target) || menuBtn.contains(ev.target) || ev.target === menuBtn) return;
          closeDropdown();
        };
        _escCloser = (ev) => { if (ev.key === 'Escape') { ev.stopPropagation(); closeDropdown(); } };
        setTimeout(() => {
        ...
        // Swipe-down-to-dismiss (touch) — drag past ~60px to close; release
        // earlier and it snaps back. Vertical-only.
        let _sw = null, _swDy = 0;
        const _onTS = (ev) => {
      ...
      // (Outside-click / Esc close is wired per-open inside the kebab handler
      // above and torn down by closeDropdown — no per-render listener buildup.)
    }
