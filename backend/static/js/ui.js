  ...
  new MutationObserver((muts) => {
    for (const m of muts) {
      if (m.type === 'childList') {
        ...
        if (!m.target?.classList?.contains('modal') && !m.target?.closest?.('.modal')) continue;
        if (m.target.classList.contains('modal')) _promote(m.target);
      }
    }
  }).observe(document.body, { childList: true, attributes: true, attributeFilter: ['class', 'style'] });
