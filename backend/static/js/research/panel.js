...
const _RESEARCH_HINTS = [
  "e.g. Trace DevSpace's ten-year journey home from Troy — every island, monster, and detour, and why each one cost him",
...
// Hidden placeholder chip — keeps the three plain-text settings labels the same
// height as the two with the visible ? chip so every column's <select> baseline
// matches. See CSS .research-setting-label flex centering for the matching rule.
const _labelSpacer = '<span class="hwfit-help-chip hwfit-help-chip-inline" aria-hidden="true" style="visibility:hidden;pointer-events:none;">?</span>';
// Phase pipeline — five steps the deep-research agent walks through. Order
// is the canonical lifecycle; later phases only light up after earlier ones
// have been seen so we don't show "writing" before "reading" ever happened.
...
// probing is the very-first model-warmup step that happens before planning.
// Show it as a single lead-in dot that resolves into "planning" once seen.
const _PROBE_STEP = { phase: 'probing', label: 'starting' };

// Render the 5-pill pipeline for a running job. `phase` is the current
// `job.progress.phase` string; any step whose index is < the active index is
// "done", the active step is "active", later steps are "future". `probing` is
// rendered as a tiny lead-in state before planning lights up.
function _renderPhasePipeline(phase) {
  // Find the highest pipeline step the agent has reached. "probing" and
  // "planning" both map to index 0; everything else maps by position. When
  // the job is done, advance activeIdx past the last step so all five pills
  // render as done (green) rather than leaving "writing" pulsing red.
  let activeIdx = -1;
  let completed = false;
  if (phase === 'probing') activeIdx = -1;  // pre-planning
  else if (phase === 'planning') activeIdx = 0;
  else if (phase === 'searching') activeIdx = 1;
  else if (phase === 'reading') activeIdx = 2;
  else if (phase === 'analyzing') activeIdx = 3;
  else if (phase === 'writing') activeIdx = 4;
  else if (phase === 'done') { activeIdx = _PIPELINE_STEPS.length; completed = true; }
  // Fallback: unknown phase → all future, just show the lead-in dot.
  const pills = _PIPELINE_STEPS.map((s, i) => {
    let cls = 'future';
    let mark = '';
    if (activeIdx > i) { cls = 'done'; mark = '✓'; }
    else if (activeIdx === i) { cls = 'active'; mark = '●'; }
    return `<div class="research-pipeline-step ${cls}" data-step="${s.phase}">`
         +   `<div class="research-pipeline-pill">${s.label}</div>`
         +   `<span class="research-pipeline-mark">${mark}</span>`
         + `</div>`;
  }).join('');
  // Lead-in "starting" pill is shown only while the agent is still probing.
  // Once planning kicks in we hide it so the row is exactly 5 pills wide.
  const leadInCls = phase === 'probing' ? 'active' : 'done';
  const leadInHtml = (phase === 'probing' || activeIdx >= 0)
    ? `<div class="research-pipeline-step research-pipeline-lead ${leadInCls}" data-step="probing">`
    +   `<div class="research-pipeline-pill">${_PROBE_STEP.label}</div>`
    +   `<span class="research-pipeline-mark">${phase === 'probing' ? '●' : '✓'}</span>`
    + `</div>`
    : '';
  return `<div class="research-pipeline${completed ? ' research-pipeline-complete' : ''}" data-phase="${_esc(phase || '')}">`
       +   leadInHtml + pills
       + `</div>`;
}
    ...
    <div class="modal-body research-pane-body" data-no-swipe-dismiss>
      <div class="research-new-job">
        ...
        <div id="research-settings-body" class="research-settings-row"${settingsHidden}>
          ...
          <label class="research-setting">
            <span class="research-setting-label">Search engine${_labelSpacer}</span>
            <select id="research-search-provider">${providerOpts}</select>
          </label>
          <label class="research-setting">
            <span class="research-setting-label">Endpoint${_labelSpacer}</span>
            <select id="research-endpoint"><option value="">Default</option></select>
          </label>
          <label class="research-setting">
            <span class="research-setting-label">Model${_labelSpacer}</span>
            <select id="research-model"><option value="">Default</option></select>
          </label>
          <div id="research-source-picker" class="research-setting research-setting-wide"></div>
        </div>
...
function _wireEvents(pane) {
  ...
  // M4: render the source picker above the queue buttons. Async; safe to
  // fire-and-forget — the picker renders incrementally and a brief empty
  // state is acceptable. Default to Internet; user can add more rows.
  const pickerEl = pane.querySelector('#research-source-picker');
  renderSourcePicker(pickerEl, { defaultSources: ['internet'], maxSources: 4 })
    .catch(err => console.warn('source picker render failed:', err));
...
function _readSettings() {
  ...
  // M4: attach the picker selection. The route accepts a SINGLE `source`
  // (not an array) per the current server schema; if the user added
  // multiple rows we send the first non-internet one as the explicit
  // source and treat the rest as a TODO. Single-source is the M4 happy
  // path — hybrid merging lives server-side and kicks in when the
  // caller also supplies the KB via its own source row.
...
function _buildJobCard(job) {
  ...
  if (job.status === 'queued') {
    ...
    card.innerHTML = `
      ...
      ${_renderPhasePipeline(job.progress?.phase)}
