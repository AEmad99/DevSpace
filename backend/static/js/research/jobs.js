...
export function init(apiBase) {
  ...
  // agent-started jobs never appeared until a page reload. The callback
  // is a no-op when no jobs are active, so the timer sits idle (1 wake
  // every 12s) instead of burning 2 fetches/min on an empty panel.
  if (_activePollInterval) clearInterval(_activePollInterval);
  _activePollInterval = window._dsTrackInterval
    ? window._dsTrackInterval(setInterval(() => {
        const _hasActive = _jobs.some(j => j.status === 'running' || j.status === 'pending');
        if (_hasActive) _reconnectActive();
      }, 12000))
    : setInterval(() => {
        const _hasActive = _jobs.some(j => j.status === 'running' || j.status === 'pending');
        if (_hasActive) _reconnectActive();
...
export function removeJob(id) {
  ...
  if (idx >= 0) {
    const job = _jobs[idx];
    if (job._es) { try { job._es.close(); } catch (_) {} job._es = null; }
