...
export default function createResearchSynapse(container, opts = {}) {
  ...
  // Compact variant (used in the per-job running card) gets bigger circles so
  // the graph reads at a glance — the full-size variant keeps its default radii.
  const rootR = opts.compact ? 13 : 11;
  const subR  = opts.compact ? 9  : 7;
  const leafR = opts.compact ? 6  : 4;
  const pulseR = opts.compact ? 7 : 6;
  const labelDy = opts.compact ? 32 : 28;   // root label sits a bit further below the root in compact
  wrap.innerHTML = `
    <div class="rs-stage">
      <svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet">
        <g class="rs-edges"></g>
        <g class="rs-nodes"></g>
        <circle class="rs-pulse" cx="${cx}" cy="${cy}" r="${pulseR}"></circle>
      </svg>
    </div>
    <div class="rs-meta">
  ...
  rootLabel.setAttribute('y', cy + labelDy);
  ...
  function _addSub(label) {
    ...
    n.setAttribute('cx', x); n.setAttribute('cy', y); n.setAttribute('r', subR);
    n.setAttribute('class', 'rs-node rs-node-sub rs-node-new');
    nodesG.appendChild(n);

    // Sub-labels are skipped in compact mode — they were the unreadable 8-9px
    // text overlaid on the tiny graph. Phase pipeline up top carries the labels.
