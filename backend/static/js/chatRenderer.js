...
import { openExternalUrl, openResearchReport } from './nativeDialog.js';
...
// Same safety contract as safeDisplayImageSrc, for model-emitted audio/video.
// Accepts same-origin/http(s) URLs (our /api/generated-image/... serving path)
// and data: URLs of the matching kind.
export function safeDisplayMediaSrc(raw, kind) {
  const src = String(raw || '').trim();
  if (!src) return '';
  const k = kind === 'video' ? 'video' : (kind === 'audio' ? 'audio' : 'image');
  if (new RegExp('^data:' + k + '\\/[a-z0-9.+-]+;base64,[a-z0-9+/=\\s]+$', 'i').test(src)) {
    return src;
  }
  try {
    const parsed = new URL(src, window.location.origin);
...
function _appendReportButton(container, sessionId) {
  ...
  btn.addEventListener('click', function() {
    openResearchReport(sessionId, apiBase);
  });
...
// Shared download + delete footer for model-emitted audio/video bubbles.
function _mediaBubbleFooter(url, label, mediaId, kind) {
  const footer = document.createElement('div');
  footer.className = 'msg-footer';
  const actions = document.createElement('span');
  actions.className = 'msg-actions';
  ...
  dlBtn.addEventListener('click', async (e) => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      const ext = (String(url).split('.').pop() || (kind === 'video' ? 'mp4' : 'mp3')).split('?')[0];
      a.download = (label || kind).slice(0, 40).replace(/[^a-zA-Z0-9 ]/g, '') + '.' + ext;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(a.href);
  ...
  if (mediaId) {
    ...
    delBtn.className = 'footer-copy-btn footer-delete-btn';
    delBtn.type = 'button';
    delBtn.title = 'Delete ' + kind;
    delBtn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg>';
    delBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const ok = await uiModule.styledConfirm('Delete this ' + kind + '?', {
      ...
      try {
        ...
        if (!res.ok && res.status !== 404) { uiModule.showToast?.('Delete failed', 4000); return; }
        window.dispatchEvent(new CustomEvent('gallery-refresh'));
      } catch (_) { uiModule.showToast?.('Delete failed', 4000); return; }
      const w = footer.closest('.msg');
      if (w) w.remove();
    });
    actions.appendChild(delBtn);
  }
...
// Custom, theme-matched audio player (the native <audio controls> is a white
// browser widget that clashes with the dark UI). Uses CSS vars with fallbacks.
function _buildCustomAudioPlayer(src) {
  ...
  player.style.cssText = 'display:flex;align-items:center;gap:10px;margin-top:6px;padding:8px 12px;background:var(--bg-elevated,var(--bg-secondary,#181b21));border:1px solid var(--border,#2a2e37);border-radius:10px;';

  const btn = document.createElement('button');
  btn.type = 'button';
  btn.className = 'media-player-btn';
  btn.style.cssText = 'width:34px;height:34px;flex:0 0 auto;border:none;border-radius:50%;background:var(--accent,#61afef);color:#0b0d10;cursor:pointer;display:flex;align-items:center;justify-content:center;';
  const PLAY = '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><polygon points="6 3 20 12 6 21 6 3"/></svg>';
  const PAUSE = '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="5" width="4" height="14" rx="1"/><rect x="14" y="5" width="4" height="14" rx="1"/></svg>';
  btn.innerHTML = PLAY;

  const bar = document.createElement('div');
  bar.className = 'media-player-bar';
  bar.style.cssText = 'flex:1;height:6px;border-radius:3px;background:var(--border,#2a2e37);cursor:pointer;position:relative;overflow:hidden;';
  const fill = document.createElement('div');
  fill.style.cssText = 'position:absolute;left:0;top:0;height:100%;width:0%;background:var(--accent,#61afef);border-radius:3px;';
  bar.appendChild(fill);

  const time = document.createElement('span');
  time.style.cssText = 'font-size:12px;color:var(--text-dim,var(--muted,#8a92a3));font-variant-numeric:tabular-nums;min-width:78px;text-align:right;flex:0 0 auto;';
  ...
  audio.addEventListener('timeupdate', () => { const d = audio.duration || 0; fill.style.width = (d ? (audio.currentTime / d * 100) : 0) + '%'; time.textContent = fmt(audio.currentTime) + ' / ' + fmt(d); });
  audio.addEventListener('loadedmetadata', () => { time.textContent = '0:00 / ' + fmt(audio.duration); });
  bar.addEventListener('click', (e) => { const r = bar.getBoundingClientRect(); const d = audio.duration || 0; if (d) audio.currentTime = ((e.clientX - r.left) / r.width) * d; });
...
/** Build a model-emitted audio bubble (themed player + transcript). */
export function buildAudioBubble(url, transcript, model, mime, mediaId) {
  const wrap = document.createElement('div');
  wrap.className = 'msg msg-ai generated-media-wrap generated-audio-wrap';
  const role = document.createElement('div');
  role.className = 'role';
  role.textContent = (model || 'audio').split('/').pop();
  wrap.appendChild(role);

  const body = document.createElement('div');
  body.className = 'body';
  const safe = safeDisplayMediaSrc(url, 'audio');
  if (!safe) {
    body.textContent = '[Audio unavailable]';
    wrap.appendChild(body);
    return wrap;
  }
  body.appendChild(_buildCustomAudioPlayer(safe));
  if (transcript) {
    const cap = document.createElement('div');
    cap.className = 'generated-image-caption generated-media-caption';
    cap.textContent = transcript;
    body.appendChild(cap);
  }
  wrap.appendChild(body);
  wrap.appendChild(_mediaBubbleFooter(safe, transcript || 'audio', mediaId, 'audio'));
  return wrap;
}

/** Build a model-emitted video bubble (with player). */
export function buildVideoBubble(url, model, mime, mediaId, poster) {
  const wrap = document.createElement('div');
  wrap.className = 'msg msg-ai generated-media-wrap generated-video-wrap';
  const role = document.createElement('div');
  role.className = 'role';
  ...
  const body = document.createElement('div');
  body.className = 'body';
  const safe = safeDisplayMediaSrc(url, 'video');
  if (!safe) {
  ...
  video.src = safe;
  if (poster) { const p = safeDisplayImageSrc(poster); if (p) video.poster = p; }
  video.className = 'generated-video';
  video.style.cssText = 'max-width:100%;border-radius:6px;margin-top:4px';
  body.appendChild(video);
  wrap.appendChild(body);
  wrap.appendChild(_mediaBubbleFooter(safe, 'video', mediaId, 'video'));
  return wrap;
}

/**
 * Dispatch a model-output media reference (from metadata.model_outputs or a live
 * model_media SSE event) to the right bubble builder.
 */
export function buildMediaBubble(mo) {
  if (!mo || !mo.url) return null;
  if (mo.media === 'audio') return buildAudioBubble(mo.url, mo.transcript, mo.model, mo.mime, mo.id);
  if (mo.media === 'video') return buildVideoBubble(mo.url, mo.model, mo.mime, mo.id, mo.poster);
  return buildImageBubble(mo.url, mo.transcript || mo.prompt, mo.model, mo.size, mo.quality, mo.id);
}
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'DevSpace';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    if (role === 'assistant' && metadata && metadata.tool_events && metadata.tool_events.length > 0) {
      ...
      for (let r = 0; r < maxRound; r++) {
        ...
        if (roundTools.length > 0) {
          ...
          for (const ev of roundTools) {
            ...
            const evDiffHtml = buildAgentDiffHtml(ev.diff, esc);
            ...
            node.innerHTML = `<div class="agent-thread-dot"></div><div class="agent-thread-header"><span class="agent-thread-icon">${ok ? '\u2713' : '\u2717'}</span><span class="agent-thread-tool">${esc(ev.tool === 'bash' ? 'Shell' : ev.tool)}</span><span class="agent-thread-status">${ok ? 'done' : 'failed'}</span><span class="agent-thread-chevron">\u25B6</span></div><div class="agent-thread-content">${evCmdHtml}${outHtml}${evDiffHtml}</div>`;
            // Click handling is delegated globally \u2014 see chat.js init.
            threadWrap.appendChild(node);
            // Phase 0 scaffold: inert hook calls (no-ops until Phase 3/7 fill them).
            if (ev.diff && ev.diff.text) _attachDiffApprovalButtons(node, ev.diff);
            highlightToolOutput(node);
          }
      ...
      // Native model-emitted media (image/audio/video) persisted on the message.
      if (Array.isArray(metadata?.model_outputs)) {
        for (const mo of metadata.model_outputs) {
          const mb = buildMediaBubble(mo);
          if (mb) { box.appendChild(mb); lastWrap = mb; }
        }
      }
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'DevSpace' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
    ...
    // Native model-emitted media (image/audio/video) persisted on the message —
    // rendered as sibling bubbles below the text, surviving reload.
    if (role === 'assistant' && Array.isArray(metadata?.model_outputs)) {
      for (const mo of metadata.model_outputs) {
        const mb = buildMediaBubble(mo);
