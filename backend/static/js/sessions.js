...
import { cancelBackgroundStream } from './chat.js';
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'DevSpace Chat';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    currentSessionId = id;
    // Notify listeners (e.g. chat.js edit-pending chip) that the active
    // session just changed, so they can refresh per-session state.
    try {
      document.dispatchEvent(new CustomEvent('devspace:session-changed', { detail: { id } }));
    } catch (_) {}
    ...
    // URL hash — the user complained that coming back to DevSpace kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'DevSpace Chat';
    }
  ...
  try {
    ...
    for (const item of items) {
      ...
      card.addEventListener('click', (e) => {
        ...
        openResearchReport(item.id, API_BASE);
      });
      ...
      if (menuBtn) {
        menuBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          _showDropdown(e.currentTarget, [
            { label: 'Open Report', action: () => openResearchReport(item.id, API_BASE) },
            { label: 'Re-run', action: () => {
