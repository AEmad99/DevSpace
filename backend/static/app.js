// ============================================
// DevSpace UI — Main Application Orchestrator
// ES6 module — entry point, no exports (wires all modules together)
// ============================================
import Storage from './js/storage.js';
import { setUserAvatar } from './js/userAvatar.js';
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'DevSpace Chat';
  ...
  if (notesModule && notesModule.refreshDueBadge) {
    ...
    window._dsTrackInterval(setInterval(() => notesModule.refreshDueBadge(), 5 * 60 * 1000));
  }
  ...
  fetch(`${API_BASE}/api/auth/status`, { credentials: 'same-origin' })
    ...
    .then(d => {
      ...
      if (userBarName && d.username) {
        ...
        if (userBarAvatar) setUserAvatar(userBarAvatar, d.username);
      }
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModeToggle() {
    ...
    const state = loadToggleState();
    // Default to 'agent' so a fresh user gets the full toolset (bash, file
    // edit, git, web search, etc.) out of the box. The previous default of
    // 'chat' skipped the agent loop entirely (tools=None in chat mode),
    // which made the agent look broken when users asked for code/file work.
    // Returning users with a saved preference keep their choice — only first
    // visits (no localStorage entry) get the new default.
  ...
  // Speak-reply (audio output) toggle — self-wired, available in both chat and
  // agent mode. Defaults off each session; backend no-ops for non-audio models.
  (function _wireSpeakToggle() {
    const sb = el('speak-toggle-btn');
    const sc = el('speak-toggle');
    if (!sb || !sc) return;
    sb.addEventListener('click', () => {
      sc.checked = !sc.checked;
      sb.classList.toggle('active', sc.checked);
      sb.setAttribute('aria-pressed', String(sc.checked));
    });
  })();
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message DevSpace...');
      }
  ...
  (function initModalMinimize() {
    ...
    new MutationObserver((muts) => {
      for (const m of muts) {
        ...
        const after = document.body.className.split(/\s+/);
        const relevant = c => c && /sidebar|dock|modal|split/.test(c);
        const changed =
          before.some(c => relevant(c) && !after.includes(c)) ||
          after.some(c => relevant(c) && !before.includes(c));
        if (changed) { updateDockOffset(); return; }
      }
...
function startDevSpaceApp() {
  ...
  window._dsTrackInterval(setInterval(_syncRailDynamic, 1000));
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startDevSpaceApp, { once: true });
} else {
  startDevSpaceApp();
}
