...
class DeepResearcher:
    ...
    def __init__(
        ...
        # --- Research sources (M1 refactor) ----------------------------
        # When `sources` is None we build a single InternetSource that
        # delegates back to this DeepResearcher's existing _search /
        # _fetch_and_extract methods. This means subclass overrides of
        # those methods (see tests/test_deep_research_extraction_controls.py)
        # continue to be honored, and all the existing side effects on
        # `self.urls_fetched` / `self.analyzed_urls` / `self.providers_used`
        # / `self._last_search_error` keep working unchanged.
        if sources is None:
            from src.research_sources.internet import InternetSource
            sources = [InternetSource(delegate=self)]
    ...
    async def research(
        ...
        for round_num in range(1, self.max_rounds + 1):
            ...
            # SEARCH + EXTRACT — routed through the Source list (M1).
            # `self.sources` defaults to a single InternetSource(delegate=self)
            # which delegates back to _search_and_extract, so behavior is
            # identical when only the internet source is configured. When
            # future sources (folder, codebase, kb) are added in M2–M4 they
            # slot into this list and their findings get merged in.
            round_findings = await self._retrieve_via_sources(queries, question)
            if round_findings:
    ...
    # SEARCH + EXTRACT
    # ------------------------------------------------------------------
    async def _retrieve_via_sources(
        self,
        queries: List[str],
        question: str,
    ) -> List[Dict]:
        """Run every registered Source's `retrieve()` and flatten results.

        Single-source mode (M1/M2 default): each Finding is converted back
        to the legacy dict shape via `Finding.to_legacy_dict()` and the
        downstream pipeline consumes the same `{url, title, summary, ...}`
        dicts it always has — zero churn.

        Multi-source mode (M4 hybrid): findings are merged with
        `hybrid.merge_findings()` so duplicates collapse and local hits get
        a small boost when the user's question signals local intent.

        A failing source MUST NOT abort the whole research session;
        failures are logged and the remaining sources' findings are
        returned. This matters once M2+ adds local sources that can fail
        on bad paths.
        """
        prior_refs = list(self.urls_fetched)
        per_source_results: List[List] = []   # one list per source
        # Track which sources are the "primary" (default) vs. extra — when
        # there's only one source we skip the merge step entirely (it
        # would be a no-op pass-through anyway).
        for src in self.sources:
            try:
                await src.warmup()
            except Exception as e:
                logger.warning(f"Source {src.type_id} warmup failed: {e}")
                per_source_results.append([])
                continue
            try:
                findings = await src.retrieve(
                    queries,
                    question=question,
                    limit=self.max_urls_per_round * len(queries),
                    prior_refs=prior_refs,
                )
            except Exception as e:
                logger.warning(f"Source {src.type_id} retrieve failed: {e}")
                self._emit(phase="warning",
                           message=f"Source '{src.type_id}' failed: {e}")
                findings = []
            finally:
                try:
                    await src.shutdown()
                    ...
                    pass
            per_source_results.append(findings)

        # Single-source path: convert straight to legacy dicts (no merge).
        if len(per_source_results) == 1:
            return [f.to_legacy_dict() for f in (per_source_results[0] or [])]

        # Hybrid merge path (M4+): de-dupe, re-rank, lightly boost local.
        from src.research_sources.hybrid import merge_findings, to_legacy_dicts
        limit = self.max_urls_per_round * len(queries)
        merged = merge_findings(
            per_source_results, question=question, limit=limit,
        )
        return to_legacy_dicts(merged)
