...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            "name": "manage_todos",
            "description": "Maintain YOUR OWN working checklist for a multi-step task (coding or otherwise). At the START of any non-trivial task, break it into concrete steps and call this with the full list; then call it again to update statuses as you complete each step. Exactly ONE item should be `in_progress` at a time; mark a step `done` the moment it's finished, then start the next. Always send the COMPLETE list (not a diff). The list renders live in the UI and is re-stated to you if a long task auto-continues, keeping you on-track across many steps. Skip it for trivial one-step requests.",
            "parameters": {
                "type": "object",
                "properties": {
                    "todos": {
                        "type": "array",
                        "description": "The COMPLETE checklist, in order. One object per concrete step.",
                        "items": {
                            "type": "object",
                            "properties": {
                                "text": {"type": "string", "description": "Short imperative description of the step."},
                                "status": {"type": "string", "enum": ["pending", "in_progress", "done"], "description": "Current status. Keep exactly one item in_progress."}
                            },
    ...
    },
    {
        ...
        "function": {
            "name": "spawn_agent",
            "description": "Delegate a self-contained sub-task to a focused SUB-AGENT that runs in its OWN context window with its own tools, then returns just a concise report. Use this to keep heavy or noisy work out of your context: exploring/searching a large codebase to answer a question, or executing one independent chunk of a bigger task. The sub-agent runs autonomously to completion (it will NOT ask questions) — so give it a complete, unambiguous `prompt` and tell it exactly what to report back. Prefer this over doing a big exploration inline when you only need the conclusion. Sub-agents cannot spawn their own sub-agents. Do NOT use for trivial single steps you can do directly.",
            "parameters": {
                "type": "object",
                "properties": {
                    "description": {"type": "string", "description": "A short (3-6 word) label for what this sub-agent is doing, e.g. 'find auth middleware'."},
                    "prompt": {"type": "string", "description": "The full, self-contained instruction for the sub-agent: what to do AND exactly what to report back. It has no access to this conversation, so include every detail it needs."},
                    "agent_type": {"type": "string", "enum": ["explore", "code", "general"], "description": "explore = read-only search/investigation (read/grep/glob/ls/web); code = read+write+edit+tests; general = full toolset. Default general."}
                },
                "required": ["prompt"]
            }
        }
    ...
    # --- Code-quality tools (Phase 0 stubs; Phase 4 implements) ---
    {
        ...
        "function": {
            "name": "run_tests",
            "description": "Run the project's test suite. Auto-detects the runner (pytest, npm test, etc.) in the workspace. Returns streamed output and the exit code.",
            "parameters": {
                "type": "object",
                "properties": {
                    "target": {"type": "string", "description": "Optional test path or filter (e.g. a file or pytest node id); empty = whole suite"}
                },
                "required": []
            }
        }
    },
    {
        ...
        "function": {
            "name": "lint",
            "description": "Run the project's linter. Auto-detects the tool (ruff, eslint, etc.) in the workspace. Returns findings and the exit code.",
            "parameters": {
                "type": "object",
                "properties": {
                    "target": {"type": "string", "description": "Optional path to lint; empty = whole project"}
                },
                "required": []
            }
        }
    },
    {
        ...
        "function": {
            "name": "format",
            "description": "Run the project's code formatter in place. Auto-detects the tool (black, prettier, etc.) in the workspace. Returns a summary of changes.",
            "parameters": {
                "type": "object",
                "properties": {
                    "target": {"type": "string", "description": "Optional path to format; empty = whole project"}
                },
                "required": []
            }
        }
    },
    # ── Git tools (workspace-scoped) ───────────────────────────────────────
    {
        "type": "function",
        "function": {
            "name": "git_status",
            "description": "Show the workspace's git working-tree status: current branch, ahead/behind, and every modified/staged/untracked file. Faster and more structured than `bash git status`. Use this before any non-trivial change so you know what you're touching.",
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {"type": "string", "description": "Optional sub-path inside the workspace to scope the status to."}
                },
                "required": []
            }
        }
    },
    {
        ...
        "function": {
            "name": "git_diff",
            "description": "Show changes in the workspace as a unified diff. By default returns the working-tree diff (unstaged + untracked content). Set staged=true to see what's already staged. For an untracked file, returns its content as a virtual + diff. Cheaper and more structured than `bash git diff`.",
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {"type": "string", "description": "Optional sub-path inside the workspace to scope the diff to."},
                    "staged": {"type": "boolean", "description": "If true, show the staged (--cached) diff instead of the working-tree diff. Default false."}
                },
                "required": []
            }
        }
    },
    {
        ...
        "function": {
            "name": "git_log",
            "description": "Show recent commits in the workspace. Default one-line format (hash, date, author, subject). Pass oneline=false for full commit bodies, or scope to a sub-path. Use this for 'what changed recently' / 'who wrote this' / 'what was the last commit touching X'.",
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {"type": "string", "description": "Optional sub-path inside the workspace to scope the log to."},
                    "max_count": {"type": "integer", "description": "Max commits to return (1..100). Default 10."},
                    "oneline": {"type": "boolean", "description": "If true (default), one line per commit. If false, include full commit body."}
                },
                "required": []
            }
        }
    },
    {
        ...
        "function": {
            "name": "git_blame",
            "description": "Annotate lines of a file with the commit/author/date that last touched them. Use this to answer 'who wrote this line / when was it last changed' without a full grep across history.",
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {"type": "string", "description": "Path of the file to blame (relative to the workspace, or absolute inside it). REQUIRED."},
                    "start_line": {"type": "integer", "description": "First line to blame (1-based). Default 1."},
                    "end_line": {"type": "integer", "description": "Last line to blame. Default = end of file."}
                },
                "required": ["path"]
            }
        }
    },
    {
        ...
        "function": {
            "name": "git_commit",
            "description": "Stage the given paths (or all modified files if `paths` is empty) and commit with a single-line message. For multi-line messages or amend/fixup flows, use the `bash` tool with `git -C <workspace> commit …` instead. Refuses to commit when there's nothing to commit and surfaces that as a clean exit (not an error).",
            "parameters": {
                "type": "object",
                "properties": {
                    "message": {"type": "string", "description": "Single-line commit message. REQUIRED."},
                    "paths": {"type": "array", "items": {"type": "string"}, "description": "Optional list of workspace-relative paths to stage before committing. Empty = stage all modified files."}
                },
                "required": ["message"]
            }
        }
    },
    {
        ...
        "function": {
            "name": "git_branch",
            "description": "Branch operations in the workspace. action='list' (default) returns all branches with the current one marked; 'current' returns just the current branch name; 'create' makes a new branch and checks it out; 'delete' removes a branch (refuses to delete unmerged branches; force-delete via `bash` if needed).",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string", "enum": ["list", "current", "create", "delete"], "description": "Which branch operation. Default 'list'."},
                    "name": {"type": "string", "description": "Branch name (required for create/delete; must not start with '-')."}
                },
                "required": []
            }
        }
    },
    {
        ...
        "function": {
            ...
            "description": "One-call orientation for a workspace: detects the project type (Python/Node/Go/Rust/...), package manager, preferred test runner / linter / formatter, entry points, conventions, and any opencode/Claude-Code instruction files (AGENTS.md, CLAUDE.md, .opencode/instructions.md). Call this at the start of a new coding task — replaces 5+ read_file/ls/glob round trips. Cached per workspace (mtime-keyed), so repeated calls in the same session are free.",
            "parameters": {
                "type": "object",
                "properties": {},
                "required": []
...
def function_call_to_tool_block(name: str, arguments: str) -> Optional[ToolBlock]:
    ...
    if tool_type == "bash":
        ...
        content = ""
    elif tool_type in ("run_tests", "lint", "format"):
        # Code-quality tools (Phase 0 stubs; Phase 4 implements).
        content = json.dumps(args) if args else "{}"
