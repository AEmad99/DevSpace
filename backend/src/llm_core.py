...
def close_http_client() -> None:
    """Close the shared httpx client. Safe to call multiple times and
    from any context (with or without a running event loop)."""
    global _http_client
    client = _http_client
    _http_client = None
    if client is None:
...
def _detect_provider(url: str) -> str:
    ...
    if _host_match(url, "minimax.io"):
        # MiniMax serves two API surfaces from the same host — a
        # Anthropic-compatible /anthropic/v1/messages and a plain
        # OpenAI-compatible /v1/chat/completions — and each one has its own
        # prompt-cache mechanism (see _is_minimax_anthropic_compat_url). The
        # provider branch in stream_llm / llm_call re-checks the path so a
        # user pointing at the anthropic-compat URL still gets the cache_control
        # builder instead of the generic openai-compat payload.
        return "minimax"
...
def _is_minimax_anthropic_compat_url(url: str) -> bool:
    """True for a MiniMax URL pointing at the Anthropic-compatible surface
    (``/anthropic/v1/...``) rather than the plain OpenAI-compatible one
    (``/v1/...``). MiniMax hosts both on the same origin (api.minimax.io),
    and they have distinct prompt-cache mechanisms:

    * Anthropic-compat: explicit ``cache_control: {type: ephemeral}`` markers
      on the system + tools blocks (re-uses the existing Anthropic builder).
    * OpenAI-compat:    passive prefix-cache (no client directives required,
      just keep the system + tools prefix stable) plus an optional
      ``prompt_cache_key`` hint.

    Misclassifying an /anthropic URL as OpenAI-compat silently drops the
    cache_control markers and the M2.x cache savings; misclassifying the
    OpenAI-compat URL as Anthropic would build the wrong payload shape (no
    ``messages`` key, different tool schema). Path-based check, not host-based,
    because the host is the same.
    """
    if not url or not _host_match(url, "minimax.io"):
        return False
    try:
        path = (urlparse(url).path or "").lower()
    except Exception:
        return False
    return "/anthropic" in path


def _normalize_minimax_openai_url(url: str) -> str:
    """Collapse a MiniMax OpenAI-compat base URL to ``/v1/chat/completions``.

    Accepts:
      * ``https://api.minimax.io``              → ``/v1/chat/completions``
      * ``https://api.minimax.io/v1``            → ``/v1/chat/completions``
      * ``https://api.minimax.io/v1/chat/completions`` → unchanged
    """
...
def _normalize_minimax_anthropic_url(url: str) -> str:
    """Collapse a MiniMax Anthropic-compat base URL to ``/anthropic/v1/messages``.
    ...
    """
    base = (url or "").strip().rstrip("/")
    for suffix in ("/anthropic/v1/messages", "/anthropic/messages", "/v1/messages"):
        if base.endswith(suffix):
            # /v1/messages is shared with native Anthropic URLs; only collapse
            # to /anthropic/v1/messages when the path was already /anthropic-*.
            if suffix == "/v1/messages" and "/anthropic" not in base.lower():
                return base
            return base
...
def _prompt_cache_enabled() -> bool:
    """Global opt-out for any upstream prompt-cache directive we inject.

    The Anthropic ``cache_control`` markers and MiniMax ``prompt_cache_key``
    are safe to send (MiniMax honors the OpenAI-standard field, Anthropic
    silently no-ops on models that lack the feature), but if an upstream
    ever rejects them rather than ignoring them we want a single escape
    hatch without code edits.
    """
    flag = os.getenv("LLM_DISABLE_PROMPT_CACHE", "").strip().lower()
    return flag not in ("1", "true", "yes", "on")


def _apply_minimax_openai_cache(payload: Dict, url: str, session_id: Optional[str]) -> None:
    """Attach a stable ``prompt_cache_key`` to a MiniMax OpenAI-compat payload.

    MiniMax's OpenAI-compatible surface (``/v1/chat/completions``) does its
    prompt caching passively: it prefix-matches the request and reports
    ``prompt_tokens_details.cached_tokens`` whenever the system+tools prefix
    lines up with a prior request inside its 5-minute TTL. No client
    directives are *required*, but the OpenAI-standard ``prompt_cache_key``
    field is honored by MiniMax to bias the cache lookup toward the same
    bucket (analogous to llama.cpp's session_id slot affinity — see
    _apply_local_cache_affinity, issue #2927). Without it, MiniMax can still
    hit the cache on a clean prefix match, but routing decisions are LRU
    global, which means a busy account with parallel conversations can
    evict our prefix. With it, requests sharing the key go to the same
    bucket.

    Only fires when:
      * the URL is MiniMax (host check, not just provider check, because
        callers may pre-normalize the URL before _detect_provider is called);
      * prompt caching is not globally disabled (LLM_DISABLE_PROMPT_CACHE);
      * a session_id was provided — a per-prompt random key would defeat the
        purpose (every turn becomes a fresh bucket), so we silently no-op
        when the caller has no stable identity to pin.
    """
    if not _prompt_cache_enabled():
        return
    if not session_id:
        return
    if not _host_match(url, "minimax.io"):
        return
    if _is_minimax_anthropic_compat_url(url):
        # The Anthropic-compat surface uses _build_anthropic_payload, which
        # already attaches cache_control breakpoints — don't double-tag here.
        return
    payload.setdefault("prompt_cache_key", str(session_id))
...
def llm_call(url: str, model: str, messages: List[Dict], temperature: float = LLMConfig.DEFAULT_TEMPERATURE,
    ...
    if provider == "anthropic":
        ...
        # Two surfaces on api.minimax.io:
        #   /anthropic/v1/messages — explicit cache_control markers, M2.x.
        #   /v1/chat/completions   — passive prefix cache + prompt_cache_key
        #                            hint, M3 (M2.x also responds but the
        #                            pricing benefit requires explicit markers).
        # We branch by path so the user picks the surface via the configured
        # endpoint URL; we don't try to auto-detect from the model name.
        if _is_minimax_anthropic_compat_url(url):
            target_url = _normalize_minimax_anthropic_url(url)
            h = _build_anthropic_headers(headers)
            ...
            target_url = _normalize_minimax_openai_url(url)
            if isinstance(headers, dict):
                h.update(headers)
            payload = {
                "model": model,
    ...
    try:
        ...
        elif provider == "minimax" and _is_minimax_anthropic_compat_url(url):
            response = _parse_anthropic_response(data)
        else:
...
async def llm_call_async(
    ...
    if provider == "anthropic":
        ...
        # See llm_call for the two-surface rationale.
        if _is_minimax_anthropic_compat_url(url):
            target_url = _normalize_minimax_anthropic_url(url)
            h = _build_anthropic_headers(headers)
            payload = _build_anthropic_payload(
...
async def stream_llm(url: str, model: str, messages: List[Dict], temperature: float = LLMConfig.DEFAULT_TEMPERATURE,
                     ...
                     want_media: bool = False, audio_voice: Optional[str] = None,
                     audio_format: Optional[str] = None, owner: Optional[str] = None):
    ...
    if provider == "anthropic":
        ...
        # /anthropic/v1/messages — reuse the Anthropic builder (cache_control
        # breakpoints on system + tools) and the Anthropic SSE parser below
        # (which already reads cache_read_input_tokens and logs [anthropic-cache]).
        target_url = _normalize_minimax_anthropic_url(url)
        h = _build_anthropic_headers(headers)
        payload = _build_anthropic_payload(model, messages_copy, temperature, max_tokens, stream=True, tools=tools)
    else:
        ...
        if _is_ollama_openai_compat_url(url) and _supports_thinking(model):
            payload["think"] = False
        # MiniMax's OpenAI-compat surface: passive prefix cache + a per-session
        # prompt_cache_key hint so the same conversation lands in the same
        # cache bucket. Anthropic-compat URLs are handled by the elif above
        # (no double-tagging).
        if provider == "minimax":
            _apply_minimax_openai_cache(payload, url, session_id)
        else:
        ...
        if provider == "copilot":
            from src.copilot import apply_request_headers
            apply_request_headers(h, messages_copy)
        # Native multimodal OUTPUT: ask the model to emit image/audio when it can.
        # Image-capable models always request images; audio is opt-in (want_media).
        # Strictly gated on the model_media registry so we never send `modalities`
        # to a model that would 400 on it.
        from src.model_media import media_request_params, wants_audio_output, audio_oneshot_events
        _media_params = media_request_params(
            provider, model, want_audio=want_media,
            audio_voice=audio_voice, audio_format=audio_format)
        if _media_params:
            payload.update(_media_params)
            if wants_audio_output(_media_params):
                # Audio output is requested NON-streaming (avoids the pcm16 /
                # stream-only constraint and honours mp3/wav/opus). The helper
                # synthesises the normal SSE protocol (text delta + model_media +
                # usage + [DONE]) around a single request.
                async for _ev in audio_oneshot_events(
                    target_url, payload, h, model=model,
    ...
    # ── Anthropic streaming ──
    # Reused by MiniMax's Anthropic-compatible surface (/anthropic/v1/messages),
    # which speaks the same SSE shape and emits the same cache_read_input_tokens
    # / cache_creation_input_tokens in message_start usage. Keep the parser
    # exactly as-is — only the trigger gate broadens.
    if provider == "anthropic" or (
        provider == "minimax" and _is_minimax_anthropic_compat_url(url)
    ):
        _cache_log_tag = "minimax-cache" if provider == "minimax" else "anthropic-cache"
        _anth_input_tokens = 0
        _anth_output_tokens = 0
        # Prompt-cache telemetry from message_start.usage. Kept across
        # the stream so the final usage event can re-emit them to the
        # frontend / agent loop.
        _c_read = 0
        _c_write = 0
                    ...
                    try:
                        ...
                        if evt == "content_block_start":
                            ...
                            if _anth_block_type == "tool_use":
                                ...
                                # Dormant: no current first-party Claude model emits
                                # media. Built for wire completeness — surfaces an image
                                # block if an Anthropic-compatible endpoint returns one.
                                try:
                                    from src.model_media import (
                                        extract_media_from_anthropic_block,
                                        store_model_media as _sm_anth,
                                    )
                                    for _it in extract_media_from_anthropic_block(cb):
                                        _ref = _sm_anth(_it, model=model,
                                                        session_id=session_id, owner=owner)
                                        if _ref:
                                            yield f'data: {json.dumps({"type": "model_media", **_ref})}\n\n'
                                except Exception:
                                    pass
                            ...
                            if _c_read or _c_write:
                                logger.info(
                                    "[%s] read=%s write=%s fresh_input=%s",
                                    _cache_log_tag,
                            ...
                            if _anth_input_tokens or _anth_output_tokens:
                                ...
                                # Surface cache stats so the agent loop / UI can
                                # report them. Only present when non-zero, so
                                # upstream providers that don't include the
                                # field don't have to be special-cased.
                                if _c_read:
                                    _anth_usage["cache_read_tokens"] = _c_read
                                if _c_write:
                                    _anth_usage["cache_creation_tokens"] = _c_write
    ...
    # Native multimodal OUTPUT: collect model-emitted media (e.g. OpenRouter
    # images) across the stream; store + emit once at end-of-stream.
    from src.model_media import (
        MediaStreamAccumulator, extract_media_from_openai_delta,
        extract_media_from_openai_message, store_all,
    )
    _media_acc = MediaStreamAccumulator()
    _media_prompt = ""
    for _m in reversed(messages_copy):
        if _m.get("role") == "user":
            _mc = _m.get("content")
            if isinstance(_mc, str):
                _media_prompt = _mc[:200]
            elif isinstance(_mc, list):
                _media_prompt = " ".join(
                    seg.get("text", "") for seg in _mc
                    if isinstance(seg, dict) and seg.get("type") == "text")[:200]
            break
    ...
    def _drain_media_events():
        """Persist accumulated media to files+gallery and return SSE event
        strings carrying only url+id (never base64)."""
        if not _media_acc.has_media():
            return []
        refs = store_all(_media_acc.drain(), prompt=_media_prompt, model=model,
                         session_id=session_id, owner=owner)
        return [f'data: {json.dumps({"type": "model_media", **ref})}\n\n' for ref in refs]
            ...
            async for line in r.aiter_lines():
                ...
                if line.startswith("data:"):
                    ...
                    if data == "[DONE]":
                        ...
                        if tc_event:
                            yield tc_event
                        for _ev in _drain_media_events():
                            yield _ev
                        yield "data: [DONE]\n\n"
                        return
                        ...
                        if data.strip():
                            if data.startswith("{"):
                                ...
                                if "usage" in j and not _delta_has_output:
                                    ...
                                    # OpenAI-compat prompt-cache stats: MiniMax exposes the
                                    # cached-prefix token count under
                                    # `prompt_tokens_details.cached_tokens` (and DeepSeek
                                    # uses `prompt_cache_hit_tokens`). Surface only the
                                    # non-zero values so providers that don't report
                                    # cache stats don't have to be special-cased.
                                    _ptd = u.get("prompt_tokens_details") or {}
                                    _cached = (
                                        _ptd.get("cached_tokens")
                                        or u.get("cached_tokens")
                                        or u.get("prompt_cache_hit_tokens")
                                        or 0
                                    )
                                    if _cached:
                                        _usage_data["cache_read_tokens"] = int(_cached)
                                        # Only emit on the round that first reports the
                                        # hit — keeps the log concise and avoids
                                        # double-counting in agent_loop which already
                                        # accumulates input_tokens across rounds.
                                        logger.info(
                                            "[%s] read=%s fresh_input=%s",
                                            "minimax-cache" if provider == "minimax" else "openai-compat-cache",
                                            _cached, _usage_data["input_tokens"],
                                        )
                                    if _actual_model:
                                    ...
                                    # Native media output: OpenRouter (and some
                                    # OpenAI-compat gateways) carry generated images
                                    # in delta.images or the terminal message.images.
                                    _mm = extract_media_from_openai_delta(delta)
                                    _msg0 = _c0.get("message")
                                    if isinstance(_msg0, dict):
                                        _mm = _mm + extract_media_from_openai_message(_msg0)
                                    if _mm:
                                        _media_acc.feed_many(_mm)
