...
ALWAYS_AVAILABLE = frozenset({
    ...
    "update_plan",
    # The agent's own working checklist for multi-step tasks. Always reachable
    # so it can plan + track progress on any task without RAG having to surface it.
    "manage_todos",
})
...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "edit_file": "Edit an existing file ON DISK by exact string replacement (fix a bug, change a function). Shows a diff. The tool for changing files on disk — NOT edit_document (editor panel) and NOT bash sed/heredoc. For several related file edits, prefer apply_patch.",
    "apply_patch": "Apply a multi-file Codex-style patch to disk (*** Begin Patch / *** End Patch). Use when several source-file edits belong together. Prefer this over bash redirects, heredocs, or sed.",
    "manage_bg_jobs": "List, read output from, or kill detached background bash jobs started with a #!bg marker in the current chat.",
    ...
    "git_status": "Show the workspace's git working-tree status — current branch, ahead/behind, and every modified/staged/untracked file. Use this before any non-trivial change so you know what you're touching.",
    "git_diff": "Show changes in the workspace as a unified diff. Default is the working-tree diff (unstaged + untracked content); pass staged=true for the staged/--cached diff. For an untracked file, returns its content as a virtual + diff.",
    "git_log": "Show recent commits in the workspace. Default one-line format; pass oneline=false for full commit bodies, or scope to a sub-path.",
    "git_blame": "Annotate lines of a file with the commit/author/date that last touched them. Answers 'who wrote this line / when was it last changed' without a full grep across history.",
    "git_commit": "Stage the given paths (or all modified files) and commit with a single-line message. Use for the common case; multi-line messages or amend still go through `bash git -C <workspace> commit …`.",
    "git_branch": "Branch operations: list all branches (current marked), show just the current branch, create + checkout a new branch, or delete a branch (refuses to delete unmerged).",
    "project_bootstrap": "One-call orientation for the active workspace: project type, package manager, test/lint/format commands, entry points, conventions, and any AGENTS.md / CLAUDE.md / .opencode/instructions.md files. Use at the start of a new coding task to skip 5+ read_file/ls/glob round trips.",
    ...
    "spawn_agent": "Delegate a self-contained sub-task to a focused sub-agent that runs in its own context window with its own tools and returns a concise report. Use it to keep heavy exploration or an independent chunk of work out of your main context — e.g. 'search this codebase and tell me where X is wired up', or 'implement and test module Y'. The sub-agent runs autonomously to completion and will not ask questions, so give it a complete prompt and say exactly what to report back. Sub-agents cannot spawn sub-agents. Not for trivial single steps you can just do yourself.",
    "manage_todos": "Maintain YOUR OWN working checklist for a multi-step task (coding or otherwise) — independent of plan mode. At the start of any non-trivial task, break it into concrete steps and call manage_todos with the full list; then call it again to flip each step's status (pending → in_progress → done) as you go. Pass `todos` as a list of {text, status} (status: pending|in_progress|done) or a markdown checklist. Always send the COMPLETE list, not a diff. Exactly one item should be in_progress at a time. The list shows live in the UI and is re-stated to you if a long task auto-continues, so it keeps you on-track. Use it to avoid losing the thread on big tasks; skip it for trivial one-step requests.",
