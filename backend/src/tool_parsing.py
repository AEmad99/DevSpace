...
# ---------------------------------------------------------------------------
# Pattern 7: Anthropic-style bare JSON tool call
# ---------------------------------------------------------------------------
# Some models (notably MiniMax-M3 and other Anthropic-tuned emitters) write
# a tool call as plain text using Anthropic's `tool_use` block shape:
#
#     {
#       "name": "web_search",
#       "input": {
...
#     }
#
# When the model is invoked without a working native tool_calls channel
# (chat mode, an endpoint that doesn't stream tool_calls, or a model that
# has never been trained to use the OpenAI channel), the call lands in the
# content stream as text and the agent would otherwise terminate. Recover
# these objects, convert them through the canonical
# function_call_to_tool_block, and execute them like any other call. Some
# emitters use `arguments` instead of `input`; accept either.
#
# We only match a JSON object whose top-level keys are STRICTLY a subset of
# {"name", "input", "arguments"} and that contains `name` + at least one of
# the arg keys. This avoids grabbing a normal JSON literal the model
# happened to write (e.g. `{"ok": true}` or any payload with unrelated
# keys). The regex is a fast pre-filter; raw_decode is the source of truth.
_ANTHROPIC_TOOL_OBJECT_RE = re.compile(
    r"\{[^{}\"']*?\"name\"\s*:\s*\"[A-Za-z_]\w*\"[^{}\"']*?"
    r"(?:\"(?:input|arguments)\"\s*:|\})",
    re.DOTALL,
)
_ANTHROPIC_TOOL_ALLOWED_KEYS = {"name", "input", "arguments"}


def _anthropic_object_to_tool_block(payload) -> Optional[ToolBlock]:
    """Convert a parsed JSON object that looks like an Anthropic tool_use block.

    Returns a ToolBlock ready for the existing execution pipeline, or None if
    the payload doesn't match the shape (unknown name, missing/non-dict args,
    extra keys that suggest it's not a tool call, etc).
    """
    if not isinstance(payload, dict):
        return None
    keys = set(payload)
    if not keys.issubset(_ANTHROPIC_TOOL_ALLOWED_KEYS):
        return None
    if "name" not in keys:
        return None
    if "input" not in keys and "arguments" not in keys:
        return None
    ...
    raw_args = payload.get("input", payload.get("arguments"))
    ...
    return function_call_to_tool_block(name, args_json)


def _parse_anthropic_tool_call_object(obj) -> Optional[ToolBlock]:
    """Public entry point used by tests and the stripper helper."""
    return _anthropic_object_to_tool_block(obj)


def _scan_anthropic_tool_objects(text: str) -> List[tuple[ToolBlock, tuple[int, int]]]:
    """Find every Anthropic-shape JSON object in `text` and convert to ToolBlocks.

    Returns a list of (ToolBlock, (start, end)) pairs in the order they appear
    in the text. End is the position one past the closing `}` so the caller
    can remove the exact JSON span from the user-visible text.
    """
    if not isinstance(text, str) or not text:
        return []
    decoder = json.JSONDecoder()
    out: List[tuple[ToolBlock, tuple[int, int]]] = []
    cursor = 0
    while True:
        # Re-scan from the previous cursor so we never match the same
        # object twice. Use the regex to find the next object start.
        m = _ANTHROPIC_TOOL_OBJECT_RE.search(text, cursor)
        if not m:
            break
        start = m.start()
        try:
            ...
            cursor = start + 1
            continue
        block = _anthropic_object_to_tool_block(parsed)
        if block is not None:
            out.append((block, (start, start + end)))
            cursor = start + end
        else:
            cursor = start + 1
    return out
...
def parse_tool_blocks(text: str, skip_fenced: bool = False) -> List[ToolBlock]:
    ...
    7. Anthropic-style bare JSON tool call: {"name":"<tool>","input":{...}} (or
       with `arguments` instead of `input`). Catches emitters that learned the
       Anthropic `tool_use` shape and emit it as text when no native
       tool_calls channel is available (e.g. chat mode, or endpoints that
       don't surface tool_calls deltas).
    ...
    can't emit structured tool_calls). Patterns 6-7 — bare JSON shape
    recoveries — also stay active regardless of `skip_fenced` for the same
    reason: when a non-native model emits its tool call as JSON text, dropping
    it would lose the call entirely.
    """
    blocks = []
    ...
    # Pattern 7: Anthropic-style bare JSON tool call — tried BEFORE Pattern 6
    # because it's a more specific match. {"name": "<tool>", "input": {<args>}}
    # (or with `arguments` instead of `input`). Always active regardless of
    # `skip_fenced` — see docstring. Catches models like MiniMax-M3 that emit
    # the Anthropic `tool_use` shape as text when the native tool_calls
    # channel isn't available (chat mode, or an endpoint that doesn't stream
    # tool_calls deltas). We don't gate this on skip_fenced: the JSON shape
    # isn't an illustrative example a native model would write into prose.
    #
    # Order matters: Pattern 6 (`web_search` keyword + bare JSON) would
    # otherwise grab the *inner* object of an Anthropic-style call as its
    # own web_search invocation — once the keyword `web_search` appears
    # inside the JSON, Pattern 6's regex happily matches the inner
    # `{"query": "..."}` as a call. Running Pattern 7 first consumes the
    # whole outer object; Pattern 6 is then a strict fallback for the
    # narrow case the regex pre-filter doesn't catch.
    if not blocks:
        for block, _span in _scan_anthropic_tool_objects(text):
            blocks.append(block)

    # Pattern 6: local text-model web_search call leaked as prose + bare JSON.
    # Strictly a fallback now that Pattern 7 has first crack at any
    # `{"name": "web_search", "input": {...}}` shape. Pattern 6 still
    # matters for emitters that write the tool name in prose and follow it
    # with a bare JSON object (no `name`/`input` wrapper).
...
def strip_tool_blocks(text: str, skip_fenced: bool = False) -> str:
    ...
    Pattern 7 (Anthropic-style bare JSON) is also always stripped: when the
    model emits `{"name":"...","input":{...}}` as text and that text becomes
    a real tool call, the user should see the tool result, not the raw JSON
    ...
    # Strip Anthropic-style bare JSON tool calls FIRST. Must mirror the
    # order in parse_tool_blocks: Pattern 7 runs before Pattern 6, otherwise
    # Pattern 6's "web_search keyword + JSON" pre-filter would consume the
    # *inner* object of an Anthropic-style call as its own web_search, and
    # then Pattern 7 would see a half-stripped outer object it can't match.
    # Splicing the whole outer object in one go also leaves the user-visible
    # text clean (no orphan quote / brace fragments).
    anthropic_calls = _scan_anthropic_tool_objects(cleaned)
    if anthropic_calls:
        # Walk the spans in reverse so earlier offsets stay valid as we
        # splice. Anything parse_tool_blocks would have matched as a call
        # gets dropped from the user-visible string.
        spans = sorted({span for _block, span in anthropic_calls}, key=lambda s: s[0], reverse=True)
