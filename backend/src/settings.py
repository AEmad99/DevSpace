...
DEFAULT_SETTINGS = {
    ...
    # Code Workspace: the server-persisted root folder the in-app file
    # tree / editor / terminal cwd operate in. Distinct from the per-turn
    # agent workspace form field (which remains the agent's source of
    # truth) — this is the UI's chosen root, vetted via vet_workspace().
    # Empty string = not set; UI prompts the user to pick a folder.
    "code_workspace_root": "",
    # File edit policy for write_file / edit_file:
    #   "auto" (default) — apply immediately, keep the old text on disk so a
    #                       later Revert can roll it back. Lets the agent fly
    #                       through edits without per-step approval prompts.
    #   "strict"          — stage the edit, show the diff, write nothing to
    #                       disk until the user clicks Apply (Discard drops it).
    #                       opencode-style: safer for real code work on a
    #                       project you don't want the agent to silently change.
    # Toggle in Settings. Existing installs that explicitly set the value in
    # data/settings.json keep their saved value; only fresh installs get the
    # new auto default.
    "agent_edit_review": "auto",
    ...
    # Coding-agent autonomy. When auto-continue is on (default), the per-message
    # `agent_max_rounds` becomes a SOFT checkpoint rather than a hard stop: if the
    # agent hits it while still actively working (using tools, not yet done or
    # blocked), the loop keeps going on its own — emitting an `agent_continue`
    # marker each time it crosses a checkpoint — up to `agent_max_rounds_ceiling`.
    # This is what makes a long coding task (read→edit→test→fix, many times) run
    # to completion instead of halting mid-task and waiting for a manual Continue.
    # Set agent_auto_continue=false to restore the old stop-at-the-cap behaviour.
    # The ceiling is the real safety bound (clamped 1..400); `agent_max_tool_calls`,
    ...
    # sub-tasks out of the main context. Depth is hard-capped at one level.
    "agent_subagents_enabled": True,
    "agent_subagent_max_rounds": 25,  # per-sub-agent step cap (clamped 1..60)
