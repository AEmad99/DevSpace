...
def _publish(run: _Run, session_id: str, ev: str) -> None:
    ...
    for q in list(run.subscribers):
        try:
            ...
            logger.warning("Subscriber queue full for run %s; dropping subscriber", session_id)
...
async def _drain(session_id: str, agen: AsyncGenerator[str, None],
    ...
    try:
        ...
        _publish(run, session_id, "data: [DONE]\n\n")
    finally:
        for q in list(run.subscribers):
            try:
                q.put_nowait((None, None))
            except asyncio.QueueFull:
                logger.warning("Subscriber queue full at end-of-run for %s; dropping subscriber", session_id)
                run.subscribers.discard(q)
