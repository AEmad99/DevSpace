...
# Project bootstrap — one-call orientation (project type, test/lint/format
# commands, conventions, AGENTS.md). Saves the agent from 5+ read_file / ls
# / glob round trips on the first turn in a new workspace.
from .project_tools import ProjectBootstrapTool
...
TOOL_HANDLERS = {
    ...
    # Git tools (workspace-scoped). Same TOOL_HANDLERS shape as every other
    # tool so the dispatcher in tool_execution.py routes them by name.
    "git_status": GitStatusTool().execute,
    "git_diff": GitDiffTool().execute,
    "git_log": GitLogTool().execute,
    "git_blame": GitBlameTool().execute,
    "git_commit": GitCommitTool().execute,
    "git_branch": GitBranchTool().execute,
    # Project bootstrap (workspace-scoped). One-call orientation; cached
    # per-workspace so repeated calls in the same session are free.
    "project_bootstrap": ProjectBootstrapTool().execute,
}
...
TOOL_TAGS = {"bash", "python", "web_search", "web_fetch", "read_file", "write_file", "edit_file",
             ...
             "edit_image", "trigger_research", "manage_research",
              ...
              # Project bootstrap — one-call orientation (project type,
              # tooling, conventions, AGENTS.md). Free to call, cached.
              "project_bootstrap"}
