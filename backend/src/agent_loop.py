...
from core.platform_compat import IS_WINDOWS, find_bash, os_label
...
def _is_strict_review_mode() -> bool:
    """True iff the agent-edit review policy is set to 'strict'. Centralises
...
Return the absolute path of the active workspace folder. File tools are CONFINED to it (paths can be RELATIVE to it); the shell starts there (cwd) but is NOT sandboxed. Call this first when the user says "the project"/"the code"/"this folder" without a path, instead of asking them. No arguments.""",
    ...
    "run_tests": """\
```run_tests
{"target": "<optional test path or filter>"}
```
Run the project's test suite. Auto-detects the runner (pytest, npm test, go test, etc.) from the workspace. Pass an optional `target` to scope to a file/node. Returns the streamed output and the exit code. Use this instead of `bash` for running tests.""",

    "lint": """\
```lint
{"target": "<optional path>"}
```
Run the project's linter. Auto-detects the tool (ruff, eslint, etc.) from the workspace. Pass an optional `target` to scope to a path. Returns findings and the exit code. Use this instead of `bash` for linting.""",

    "format": """\
```format
{"target": "<optional path>"}
```
Run the project's code formatter IN PLACE. Auto-detects the tool (black, prettier, etc.) from the workspace. Pass an optional `target` to scope to a path. Returns a summary of changes. Use this instead of `bash` for formatting.""",
...
`calendar` accepts a name ("Main") or short-id prefix.""",
    ...
    "spawn_agent": "- ```spawn_agent``` — Delegate a self-contained sub-task to a focused SUB-AGENT with its own context + tools; it runs autonomously to completion and returns just a summary. Args (JSON): {\"description\": \"short label\", \"prompt\": \"full self-contained instruction incl. exactly what to report back\", \"agent_type\": \"explore|code|general\"}. Use to keep heavy exploration or an independent chunk of work out of your context (e.g. 'find where X is wired up', 'implement+test module Y'). The sub-agent can't see this chat and won't ask questions, so give it everything. Sub-agents can't spawn sub-agents. Not for trivial single steps.",
    "manage_todos": "- ```manage_todos``` — Your OWN working checklist for a multi-step task. Args (JSON): {\"todos\": [{\"text\": \"step\", \"status\": \"pending|in_progress|done\"}, ...]}. At the START of any non-trivial task (especially coding), break it into concrete steps and call this once with the full list; then call it again to update statuses as you finish each step. Keep exactly ONE item `in_progress`; mark a step `done` the moment it's complete, then move to the next. Always send the COMPLETE list. It renders live and is re-stated to you if a long task auto-continues, so it keeps you on-track — skip it only for trivial one-step requests.",
...
```git_status
{}                # whole workspace
{"path": "src/"}  # scope to a sub-path
```
Show the workspace's git working-tree status — current branch, ahead/behind, and every modified/staged/untracked file. Cheaper + more structured than `bash git status`. Call this before any non-trivial change so you know what you're touching, and after a write_file/edit_file to confirm the file is staged. If the workspace isn't a git repo, returns a clean "(not a git repository)" message.""",

    "git_diff": """\
```git_diff
{}                          # working-tree diff (unstaged + untracked)
{"staged": true}            # staged (--cached) diff
{"path": "src/foo.py"}      # scope to a sub-path
{"path": "new.txt", "staged": false}  # untracked file → virtual + diff
```
Show changes in the workspace as a unified diff. By default the working-tree diff; pass `staged: true` for `--cached`. For an untracked file, returns its full content as a virtual + diff (cheaper than reading the file then diffing manually). Use this right after an edit to confirm the change looks right, or to inspect what someone else's branch has changed.""",

    "git_log": """\
```git_log
{}                              # last 10 commits, one-line
{"max_count": 30, "oneline": true}
{"path": "src/foo.py"}          # only commits touching this path
{"oneline": false}              # full commit bodies
```
Show recent commits. Default is a compact one-line format (hash, date, author, subject). Pass `oneline: false` for full bodies. Scope to a `path` to see "what changed recently in this file". Use for "what's the latest commit", "who wrote this", "what was the last commit touching X".""",

    "git_blame": """\
```git_blame
{"path": "src/foo.py"}              # whole file
{"path": "src/foo.py", "start_line": 10, "end_line": 25}  # range
```
Annotate lines of a file with the commit/author/date that last touched them. Use this to answer "who wrote this line / when was it last changed" without a full grep across history. `start_line` and `end_line` (1-based) scope the blame to a region — useful for big files where you only care about one function.""",

    "git_commit": """\
```git_commit
{"message": "short imperative subject", "paths": ["src/foo.py"]}
{"message": "subject"}              # stage all modified files, then commit
```
Stage the given paths (or all modified files if `paths` is omitted) and commit with a single-line message. For multi-line messages, amend/fixup, signed-off-by, etc., use `bash git -C <workspace> commit …` instead. Refuses to commit when there's nothing to commit and surfaces that as a clean exit (not an error) — do NOT retry on that message.""",

    "git_branch": """\
```git_branch
{}                                  # list all branches (current marked)
{"action": "current"}               # just the current branch name
{"action": "create", "name": "feat/x"}
{"action": "delete", "name": "feat/x"}  # refuses to delete unmerged branches
```
Branch operations. `list` (default) returns every branch with the current one marked. `current` returns just the current branch name. `create` makes a new branch and checks it out. `delete` is safe by default — it refuses to delete a branch with unmerged commits; force-delete via `bash git -C <workspace> branch -D <name>` if you really mean it. Branch names must not start with `-` (a safety check, not a git limitation).""",

    "project_bootstrap": """\
```project_bootstrap
{}                                 # no args — inspects the active workspace
```
One-call orientation for the active workspace. Returns: project type + language + package manager, preferred test runner / linter / formatter commands, entry points, conventions (e.g. "uses `src/` layout", "type-checked with mypy", "has a Makefile"), and any opencode / Claude Code instruction files (AGENTS.md, CLAUDE.md, .opencode/instructions.md) with a short preview. Cached per-workspace (mtime-keyed), so repeated calls in the same session are free.

Call this at the START of any new coding task — replaces 5+ read_file / ls / glob round trips just to figure out what the project is. After this call, you'll know which test command to run, which conventions to follow, and which files to read first. Then dive in with `read_file` / `git_status` / `grep` for the actual work.""",
}
...
def _shell_env_note() -> str:
    """OS/shell grounding appended to the `bash` tool section.

    Without this the model has no idea which OS it is on and blindly emits bash,
    which fails when the command tool falls back to cmd.exe on a Windows box with
    no Git Bash. Kept in sync with platform_compat.agent_shell_invocation (the
    function that actually picks the shell)."""
    osn = os_label()
    if IS_WINDOWS and not find_bash():
        return (
            f"SHELL ENVIRONMENT: This machine is {osn} with no bash/WSL installed, so the "
            "`bash` tool runs through cmd.exe. Use Windows command syntax — `dir` (not `ls`), "
            "`type` (not `cat`), `copy`/`move`/`del`, `where` (not `which`), `findstr` (not "
            "`grep`), `%VAR%` for env vars, `&&` to chain commands. For anything beyond simple "
            "commands, call PowerShell: `powershell -NoProfile -Command \"...\"`. POSIX-only "
            "constructs (heredocs, `ls`/`cat`/`grep`, `$VAR`) will NOT work."
        )
    if IS_WINDOWS:
        return (
            f"SHELL ENVIRONMENT: This machine is {osn}, but the `bash` tool runs through Git "
            "Bash — use POSIX/bash syntax (`ls`, `cat`, `grep`, forward slashes), not "
            "cmd/PowerShell. Windows drives are reachable as `/c/Users/...`; native Windows "
            "executables (git, python, npm) still work."
        )
    return (
        f"SHELL ENVIRONMENT: This machine is {osn}; the `bash` tool runs through the system "
        "shell. Use standard POSIX/bash syntax."
    )
...
def _build_base_prompt(
    ...
    if relevant_tools is not None:
        ...
        tool_names = set(relevant_tools) | {"ask_user", "update_plan", "manage_todos"}
        if needs_admin:
...
def _compute_final_metrics(
    ...
    # Upstream prompt-cache totals (Anthropic cache_read/cache_creation,
    # MiniMax M2.x cache_creation, MiniMax M3 passive cache_read).
    # Surface them so the metrics pane / session summary can show the user
    # how much of the system+tools prefix was served from cache. Only
    # present when non-zero — most providers don't report these fields, so
    # we don't pad the dict with zeros.
...
# Tools whose use means the loop is MAKING PROGRESS (mutating state / producing
# a new artifact), so the stall detector must not count a round that used one as
# "spinning" — even if the call signature repeats. Broader than the verifier set
# because it includes disk edits and the agent's own checklist updates: an
# edit→test→edit coding loop reuses identical `edit_file`/`run_tests` signatures
# but is clearly progressing.
_PROGRESS_EFFECTFUL_TOOLS = _VERIFIER_EFFECTFUL_TOOLS | {
    "suggest_document", "manage_todos", "generate_image", "edit_image",
}

# Heuristic: does the user's message look like a CODING task? When the model
# is working on a coding problem (bug fix, feature, refactor, "add a test for
# X"), the verifier subagent catches "claimed done but didn't actually run the
# tests" cases that the human would otherwise have to catch in PR review. For
# the personal-assistant use case (notes, calendar, email) the verifier is
# mostly noise — and on weak local models it false-rejects because it can't
# "see" the doc body in the action snapshot.
#
# Heuristic: an active workspace + any of (a) the user message mentions a code
# path / file extension, (b) a turn starts with the user clicking a chat that
# has a workspace attached, or (c) the conversation has used edit_file /
# write_file in the last few turns. Cheap, no LLM call, runs once at the top
# of the turn.
_CODING_TURN_HINT_RE = re.compile(
    r"\b("
    r"fix|bug|error|exception|traceback|broken|failing|"
    r"implement|refactor|rewrite|rework|port|"
    r"compile|build|makefile|configure|setup|install|"
    r"clean\s*up|rename|move|extract|split|merge|"
    r"optimize|speed\s*up|performance|memory\s*leak|"
    r"test|tests|coverage|pytest|jest|unittest|mocha|"
    r"deploy|release|ci|cd|docker|kubernetes|k8s|"
    r"migrate|migration|upgrade|downgrade|"
    r"function|class|method|module|component|service|handler|endpoint|"
    r"import\s+\w+|from\s+\w+\s+import|"
    r"linter|lint|ruff|eslint|flake8|"
    r"compile\s*error|syntax\s*error|"
    r"\.py\b|\.js\b|\.ts\b|\.tsx\b|\.jsx\b|\.go\b|\.rs\b|\.java\b|"
    r"\.rb\b|\.php\b|\.swift\b|\.kt\b|\.c\b|\.cpp\b|\.h\b|\.hpp\b|"
    r"\.cs\b|\.sh\b|\.sql\b|\.html\b|\.css\b|\.scss\b|\.vue\b|\.svelte\b"
    r")\b",
    re.IGNORECASE,
)


def _looks_like_coding_turn(user_msg: str, *, workspace: str = None) -> bool:
    """Best-effort coding-task detection. True when either a workspace is
    active and the user message mentions a code-ish term, OR the message
    contains a file extension / class of file the agent can edit. The
    verifier only fires on a True result (when the setting is on)."""
    if not user_msg:
        return False
    if workspace and _CODING_TURN_HINT_RE.search(user_msg):
        return True
    # Even without a workspace, an obvious file-extension reference is a
    # strong coding signal — e.g. "edit src/foo.py" with no workspace set
    # is a coding task that the user hasn't bound yet.
    if re.search(r"\.\s*(py|js|ts|tsx|jsx|go|rs|java|kt|swift|c|cpp|h|hpp|cs|rb|php|sh|sql|html|css|scss|vue|svelte)\b",
...
async def stream_agent_loop(
    ...
    cache_read_tokens_total = 0   # Sum of upstream-cache reads (Anthropic/MiniMax)
    cache_creation_tokens_total = 0  # Sum of cache writes (Anthropic/MiniMax M2.x)
    ...
    _stuck_rounds = 0
    # Runaway backstop: how many times each exact call signature has repeated
    # while ALSO returning identical output (a true no-op spin). Counting only
    # unchanged-output repeats — not distinct calls, not identical calls whose
    # output keeps changing — lets a legit batch (18 calendar events) AND an
    # iterative coding loop (run_tests after each fix) both through.
    _noop_freq: collections.Counter = collections.Counter()
    # Last seen output hash per call signature, to detect "same call, new output".
    _last_out_by_sig: Dict[str, int] = {}
    # Progress signals from the PREVIOUS round's execution, consumed by the
    # loop-breaker at the top of the next round. Effectful = a file/doc/todo
    # write happened; progress = some tool returned output differing from its
    # prior identical call. Either one means the loop is moving, not spinning.
    _last_round_effectful = False
    _last_round_progress = False
    ...
    # ── Auto-continue (Claude-Code-style "don't stop mid-task") ──────────
    # `max_rounds` is the SOFT per-message budget. When auto-continue is on,
    # the loop treats it as a checkpoint, not a wall: on crossing it while
    # still actively working it emits an `agent_continue` marker, re-states
    # the open TODOs, and keeps going up to `_hard_ceiling`. The hard ceiling
    # (plus the tool-call / token / wall-clock budgets) is the real bound.
    _auto_continue = bool(auto_continue) and bool(get_setting("agent_auto_continue", True))
    if _auto_continue:
        try:
            _hard_ceiling = int(get_setting("agent_max_rounds_ceiling", 150) or 150)
        except (TypeError, ValueError):
            _hard_ceiling = 150
        _hard_ceiling = max(max_rounds, min(_hard_ceiling, 400))
    else:
        _hard_ceiling = max_rounds
    # Self-directed TODO checklist the agent maintains via `manage_todos`.
    # Persisted across rounds so it can be re-injected at each auto-continue
    # checkpoint — this is what keeps a long task on-rails instead of drifting.
    _todos_md = ""
    _continue_count = 0  # how many soft checkpoints we've auto-crossed

    for round_num in range(1, _hard_ceiling + 1):
        # Soft-checkpoint: crossed the per-message budget but still working.
        # Re-anchor the model on its open TODOs and signal the UI, then carry on.
        if _auto_continue and round_num > max_rounds and (round_num - 1) % max_rounds == 0:
            _continue_count += 1
            _open_todos = _todos_md.strip()
            _todo_note = (
                "\n\nYour current TODO list (keep working the unchecked items):\n"
                + _open_todos if _open_todos else ""
            )
            messages.append({
                "role": "system",
                "content": (
                    f"[auto-continue] You've used {round_num - 1} steps and the task "
                    "is not finished. Keep going from exactly where you left off — do "
                    "NOT restate a plan, do NOT repeat completed work, and do NOT stop "
                    "to ask whether to continue. Take the single most useful next "
                    "action now. Only finish when every concrete deliverable is done "
                    "(declare done by replying with no tool call), or say plainly that "
                    "you are blocked." + _todo_note
                ),
            })
            logger.info("[agent] auto-continue checkpoint #%d at round %d (ceiling %d)",
                        _continue_count, round_num, _hard_ceiling)
            yield f'data: {json.dumps({"type": "agent_continue", "round": round_num, "checkpoint": _continue_count})}\n\n'
        round_response = ""
            ...
            if chunk.startswith("data: ") and not chunk.startswith("data: [DONE]"):
                try:
                    ...
                    if data.get("type") == "tool_call_delta":
                        ...
                        has_real_usage = True
                        # Upstream prompt-cache stats (Anthropic / MiniMax).
                        # Accumulate so the round-summary log can report
                        # aggregate hit rates; per-round values are emitted
                        # in real time by stream_llm's [anthropic-cache] /
                        # [minimax-cache] log lines.
                        _cr = u.get("cache_read_tokens")
                        if _cr:
                            cache_read_tokens_total += int(_cr)
                        _cw = u.get("cache_creation_tokens")
                        if _cw:
                            cache_creation_tokens_total += int(_cw)
                    ...
                    elif data.get("type") == "model_media":
                        # Native model-emitted media (e.g. an image-output model
                        # used in agent mode) — forward to chat_routes verbatim.
                        yield chunk
                    elif "delta" in data:
        ...
        if not tool_blocks:
            ...
            # Verifier gate: ON by default for coding tasks (workspace + code
            # mention, or a file extension in the message), OFF for the
            # personal-assistant use case where it false-rejects on doc edits.
            # The setting overrides the default: explicit `true` forces it on
            # for every turn; explicit `false` forces it off.
            _verifier_setting = get_setting("agent_verifier_subagent", None)
            if _verifier_setting is None:
                _last_user_text = _extract_last_user_message(messages)
                _verifier_on = _looks_like_coding_turn(_last_user_text, workspace=workspace)
            else:
                _verifier_on = bool(_verifier_setting)
        ...
        # Circling = repeating a recent call with nothing written AND the last
        # executed round made no progress. The progress signals are computed
        # after execution (below): `_last_round_effectful` (a file write/edit/
        # doc/todo change happened) and `_last_round_progress` (a tool returned
        # OUTPUT that differs from the previous identical call). This is the
        # coding-loop fix: read→edit→test→edit→test re-issues the SAME
        # `run_tests`/`edit_file` signatures, but each test run yields fresh
        # output and each edit mutates a file, so it never reads as "stuck".
        # Only genuinely spinning in place (same call, same output, no edits,
        # no text) accrues the stall counter.
        if (_is_repeat and not _real_text
                and not _last_round_effectful and not _last_round_progress):
            _stuck_rounds += 1
        else:
            _stuck_rounds = 0
        # Runaway backstop = the SAME exact call returning the SAME output an
        # absurd number of times (a true no-op spin). `_noop_freq` is bumped
        # during execution only when a call repeats AND its output is unchanged,
        # so a real iterative batch (distinct calls, or identical calls whose
        # output keeps changing) is never flagged.
        _runaway = _detect_runaway_call(_noop_freq)
        ...
        budget_hit = False
        # Per-round progress signals (consumed by the loop-breaker next round).
        _round_effectful = False
        _round_progress = False
        for i, block in enumerate(tool_blocks):
            ...
            if tool_policy and tool_policy.blocks(block.tool_type):
                ...
                try:
                    ...
                    while True:
                        ...
                        if evt is None:
                            break
                        # spawn_agent forwards nested-loop progress tagged with a
                        # "subagent" key — surface those as dedicated subagent_*
                        # events so the UI can show a live sub-agent indicator.
                        _sub = evt.get("subagent") if isinstance(evt, dict) else None
                        if _sub:
                            yield (
                                f'data: {json.dumps({"type": f"subagent_{_sub}", "round": round_num, **{k: v for k, v in evt.items() if k != "subagent"}})}\n\n'
                            )
                        else:
                            yield (
            ...
            # manage_todos: agent's own working checklist. Push to the frontend
            # and persist the markdown so open items can be re-stated at each
            # auto-continue checkpoint. Does NOT end the turn.
            if "todo_update" in result:
                _todos_md = (result["todo_update"].get("markdown") or "").strip()
                yield (
                    f'data: {json.dumps({"type": "todo_update", "data": result["todo_update"]})}\n\n'
                )
            ...
            # ── edit_pending ──
            # When the tool just produced a staged (strict-mode) edit, surface a
            # lightweight `edit_pending` event so the UI can show a "1 edit
            # awaiting approval" chip on the input bar and a "View session
            # changes" affordance on the assistant message — without having to
            # walk the DOM counting unresolved diff-actions bars. The payload
            # carries the staged paths and a running count for this session.
            #
            # Auto mode is the new default — writes land on disk immediately
            # and there is nothing to approve. Skip the event entirely so the
            # frontend stays quiet (no chip, no drawer). The chat JS still
            # handles the event if it arrives (strict-mode toggle escape
            # hatch), but it just never fires in auto mode.
            if (
                isinstance(result.get("diff"), dict)
                and result["diff"].get("staged")
                and result["diff"].get("checkpoint_id")
                and (lambda: _is_strict_review_mode())()
            ):
                try:
                    ...
                    _cp = result["diff"]["checkpoint_id"]
                    _count = count_pending_for_session(session_id) if session_id else 1
                    _pending = list_pending_for_session(session_id) if session_id else [
                        {"checkpoint_id": _cp, "path": result["diff"].get("path", ""),
                         "file": result["diff"].get("file", "")}
                    ]
                    yield (
                        'data: ' + json.dumps({
                            "type": "edit_pending",
                    ...
                    logger.debug("edit_pending event skipped: %s", _e)
            ...
            # vs. the last time we ran the exact same call. Only a repeat with
            # IDENTICAL output is a no-op (counts toward the runaway backstop).
            _bsig = f"{block.tool_type}:{(block.content or '').strip()[:120]}"
            _out_hash = hash(output_text or "")
            _prev_hash = _last_out_by_sig.get(_bsig)
            if _prev_hash is None or _prev_hash != _out_hash:
                _round_progress = True
                _noop_freq[_bsig] = 0
            else:
                _noop_freq[_bsig] += 1
            _last_out_by_sig[_bsig] = _out_hash
        ...
        _last_round_progress = _round_progress
    ...
    if _exhausted_rounds:
        logger.info("[agent] round cap (%d) reached mid-task — emitting rounds_exhausted", _hard_ceiling)
        yield f'data: {json.dumps({"type": "rounds_exhausted", "rounds": _hard_ceiling})}\n\n'
