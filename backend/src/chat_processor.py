...
class ChatProcessor:
    ...
    def build_context_preface(
        ...
        *anything* that changes turn-to-turn — timestamps, retrieved snippets,
        ...
        # The app generates media for the user (auto-routed to image/audio/video
        # models) and feeds generated images back as vision input — tell the model
        # so it stops denying media capabilities it effectively has.
        preface.append({
            "role": "system",
            "content": (
                "Media generation: this application CAN produce images, audio (speech), and video. "
                "When the user asks for one, the request is automatically routed to the appropriate "
                "media model and the result is shown to the user — so never claim you are unable to "
                "generate media. Images generated earlier in the conversation may be provided back to "
                "you as image input; you can see them and may judge, score, or discuss them directly. "
                "When asked to evaluate a shown image, evaluate it rather than refusing."
            ),
        })
