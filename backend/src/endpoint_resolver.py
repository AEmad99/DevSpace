...
from src.llm_core import (
    ...
    _host_match,
    _is_kimi_code_url,
    _is_minimax_anthropic_compat_url,
    _normalize_minimax_anthropic_url,
    _normalize_minimax_openai_url,
...
def build_chat_url(base: str) -> str:
    ...
    # The provider branch in stream_llm / llm_call re-checks the path to pick
    # the right payload shape; this function only has to land on the right URL
    # for whatever the user configured (host + path). Branching here too means
    # a user who pastes a bare ``https://api.minimax.io`` or
    # ``https://api.minimax.io/anthropic`` into Model Endpoints still hits a
    # 200, instead of 404'ing on /chat/completions (a real path only the
    # OpenAI surface exposes).
    if provider == "minimax":
        if _is_minimax_anthropic_compat_url(base):
            return _normalize_minimax_anthropic_url(base)
        return _normalize_minimax_openai_url(base)
...
def build_models_url(base: str) -> Optional[str]:
    ...
    # Falling through to the generic /models append would 404.
    if provider == "minimax":
        # MiniMax's catalog lives at /v1/models on both surfaces. Strip any
        # of the well-known suffixes the user might have pasted — /v1/chat/
        # completions, /v1, /v1/messages, /anthropic/v1/messages — so the
        # resulting path is always the bare origin + /v1/models. Anything
        # else is left untouched (a custom prefix a user typed is preserved).
        parsed = urlparse(base)
        ...
        for _suffix in (
            ...
            if path == _suffix or path.endswith(_suffix):
                path = path[: -len(_suffix)].rstrip("/")
                break
        origin = urlunparse(parsed._replace(path=path, query="", fragment="")).rstrip("/")
        return _append_endpoint_path(origin, "/v1/models")
...
def build_headers(api_key: Optional[str], base: str) -> Dict[str, str]:
    ...
    # MiniMax's Anthropic-compatible surface (/anthropic/v1/messages) speaks
    # the Anthropic wire format — including the ``x-api-key`` + ``anthropic-version``
    # auth pair — even though the /v1/models listing still accepts Bearer
    # tokens. Use the Anthropic headers so the chat path doesn't 401.
    if provider == "anthropic" or (
        provider == "minimax" and _is_minimax_anthropic_compat_url(base)
    ):
        if api_key:
