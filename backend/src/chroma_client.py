...
By default this returns an embedded persistent store living under the
backend's data directory (no separate service required). Set CHROMADB_HOST
explicitly, or CHROMADB_MODE=http, to connect to a standalone ChromaDB
service over HTTP instead.
"""
...
def get_chroma_client():
    ...
    Defaults to an embedded persistent store under the backend data dir so
    RAG + memory vectors work with no separate service. Falls back to the
    HTTP client when CHROMADB_HOST is set explicitly or CHROMADB_MODE=http.
    ...
    # Embedded persistent is the default; only use HTTP when explicitly asked.
    mode = os.getenv("CHROMADB_MODE", "").strip().lower()
    use_http = mode == "http" or "CHROMADB_HOST" in os.environ
    ...
    if not use_http:
        ...
        logger.info(f"ChromaDB embedded persistent store at {path}")
        return _client
