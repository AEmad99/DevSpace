...
class ResearchHandler:
    ...
    def start_research(
        ...
        async def _run():
            try:
                ...
                # M4: detach the KB file watcher (if any) so the watchdog
                # thread terminates cleanly with the session.
                obs = entry.get("watcher")
                if obs is not None:
                    try:
                        from services.research.watcher import detach_watcher
                        detach_watcher(obs)
                        entry["watcher"] = None
                    except Exception as e:
                        logger.debug(f"watcher detach: {e}")
                entry["status"] = "error"
    ...
    async def call_research_service(
        ...
        try:
            ...
            # M4: auto-attach a file watcher if any source is a KB so the
            # index stays fresh while the research session runs.
            watcher_obs = None
            if sources and _task_entry is not None:
                for src in sources:
                    if getattr(src, "type_id", None) == "kb":
                        try:
                            from services.research.watcher import attach_watcher
                            watcher_obs = attach_watcher(src, loop=asyncio.get_running_loop())
                            _task_entry["watcher"] = watcher_obs
                            logger.info(f"Attached file watcher for KB {src.kb_id}")
                        except Exception as e:
                            logger.warning(f"Could not attach KB watcher: {e}")
                        break
