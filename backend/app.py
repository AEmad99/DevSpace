...
if AUTH_ENABLED:
    ...
    AUTH_EXEMPT_PATTERNS = [
        ...
        # Visual report opens in an EXTERNAL browser (no session cookie). The
        # handler authenticates itself from a short-lived ?t= report token (or a
        # cookie when opened in-app) and 401s on its own if neither is valid.
        # See routes/research_routes.py report-token bridge.
        _re.compile(r"^/api/research/report/[^/]+/?$"),
    ]
            ...
            if auth_header.startswith("Bearer ody_"):
                ...
                try:
                    ...
                    if matched_id:
                        ...
                        # Strong ref so the fire-and-forget task can't be GC'd
                        # before it runs. Mirrors WebhookManager._spawn_tracked.
                        if not hasattr(app.state, "_touch_tasks"):
                            app.state._touch_tasks = set()
                        _touch_task = _asyncio.create_task(_touch_last_used(matched_id))
                        app.state._touch_tasks.add(_touch_task)
                        _touch_task.add_done_callback(app.state._touch_tasks.discard)
...
# Research sources registry (issue #2 / M1) — exposes pluggable sources
# (internet today; folder / codebase / knowledge-base in M2+) to the UI.
from routes.research_sources_routes import router as research_sources_router
app.include_router(research_sources_router)

# Knowledge Base CRUD (issue #2 / M4).
...
# Code Workspace (file tree, editor, search, git, diff revert) - Phase 0 scaffold.
# OWNED BY: stream 1 (code_workspace_routes.py). Endpoints appended in Phase 1/3/6.
from routes.code_workspace_routes import setup_code_workspace_routes
app.include_router(setup_code_workspace_routes())

# LSP (Language Server Protocol) WebSocket bridge for the in-app Code editor.
# Spawns per-language language servers (pylsp for Python in v1) and proxies
# JSON-RPC frames between the editor and the server's stdio. Path-confinement
# matches the Code Workspace policy — URIs outside the workspace root are
# dropped before reaching the language server.
from routes.lsp_routes import setup_lsp_routes
app.include_router(setup_lsp_routes())
...
async def _shutdown_event():
    ...
    if upload_cleanup_task:
        ...
        try:
            ...
            pass
    # Cancel long-lived background tasks registered during startup
    # (bg-monitor, MCP connect-all, RAG pre-warm, audit/skill/cookbook loops, ...).
    for t in getattr(app.state, "_startup_tasks", []):
        if not t.done():
            t.cancel()
        try:
            await t
    ...
    # Cancel in-flight token-last_used touch tasks.
    for t in getattr(app.state, "_touch_tasks", set()):
        if not t.done():
            t.cancel()
        try:
            await t
    ...
    # Close the shared httpx client used by LLM calls.
    try:
        from src.llm_core import close_http_client
        if asyncio.iscoroutinefunction(close_http_client):
            await close_http_client()
        else:
            close_http_client()
    except Exception as e:
        logger.debug(f"llm_core http client close skipped: {e}")
