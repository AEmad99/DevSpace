...
def _anon_request():
    """A cookie-less request, like the external browser opening the report."""
    return SimpleNamespace(
        state=SimpleNamespace(current_user=None),
        app=SimpleNamespace(state=SimpleNamespace(auth_manager=None)),
        cookies={},
        client=SimpleNamespace(host="203.0.113.7"),  # non-loopback
    )


def test_report_token_authorizes_external_browser_for_owner(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    data_dir = tmp_path / "data" / "deep_research"
    _write_research(data_dir, "alice-report", owner="alice", result="alice report")

    handler = _research_handler()
    handler.get_report_html.return_value = "<html>alice report</html>"
    router = setup_research_routes(handler)

    # Owner mints a token from the (cookie-authed) WebView...
    mint = _route(router, "/api/research/report-link/{session_id}", "POST")
    minted = asyncio.run(mint(session_id="alice-report", request=_request("alice")))
    assert minted["token"] and minted["url"].startswith("/api/research/report/alice-report?t=")

    # ...then the cookie-less external browser opens the report with ?t=token.
    report = _route(router, "/api/research/report/{session_id}", "GET")
    resp = asyncio.run(report(
...
def test_report_rejects_anonymous_request_without_token(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    data_dir = tmp_path / "data" / "deep_research"
    _write_research(data_dir, "alice-report", owner="alice", result="alice report")
    ...
    with pytest.raises(HTTPException) as exc:
        asyncio.run(report(session_id="alice-report", request=_anon_request(), t=None))

    assert exc.value.status_code == 401
    handler.get_report_html.assert_not_called()


def test_report_rejects_token_minted_for_a_different_session(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    data_dir = tmp_path / "data" / "deep_research"
    _write_research(data_dir, "alice-report", owner="alice", result="alice")
    _write_research(data_dir, "alice-other", owner="alice", result="other")

    handler = _research_handler()
    router = setup_research_routes(handler)
    mint = _route(router, "/api/research/report-link/{session_id}", "POST")
    minted = asyncio.run(mint(session_id="alice-other", request=_request("alice")))
