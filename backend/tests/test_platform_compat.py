...
def test_agent_shell_invocation_windows_with_bash_uses_login_shell(monkeypatch):
    # Windows + bash present: run as a LOGIN shell ("-lc") with CHERE_INVOKING so
    # /etc/profile repairs PATH (so ls/cat/grep resolve even when bash is the
    # bare …\\usr\\bin\\bash.exe) without cd-ing away from the agent's cwd.
    monkeypatch.setattr(platform_compat, "IS_WINDOWS", True)
    monkeypatch.setattr(platform_compat, "find_bash", lambda: r"C:\Program Files\Git\bin\bash.exe")

    spec = platform_compat.agent_shell_invocation("ls -la && echo hi")

    assert spec["argv"] == [r"C:\Program Files\Git\bin\bash.exe", "-lc", "ls -la && echo hi"]
    assert spec["shell_command"] is None
    assert spec["env_extra"] == {"CHERE_INVOKING": "1"}


def test_agent_shell_invocation_windows_without_bash_uses_cmd_shell(monkeypatch):
    # Windows + no bash: fall back to the system shell (cmd.exe). No login argv,
    # no extra env — the agent is told in its prompt to use Windows syntax.
    monkeypatch.setattr(platform_compat, "IS_WINDOWS", True)
    monkeypatch.setattr(platform_compat, "find_bash", lambda: None)

    spec = platform_compat.agent_shell_invocation("dir")

    assert spec["argv"] is None
    assert spec["shell_command"] == "dir"
    assert spec["env_extra"] == {}


def test_agent_shell_invocation_posix_uses_system_shell(monkeypatch):
    # POSIX is unchanged: the system shell via create_subprocess_shell. find_bash
    # is never consulted (it's a Windows-only concern here).
    monkeypatch.setattr(platform_compat, "IS_WINDOWS", False)
    monkeypatch.setattr(
        platform_compat, "find_bash",
        lambda: (_ for _ in ()).throw(AssertionError("find_bash must not be called on POSIX")),
    )

    spec = platform_compat.agent_shell_invocation("ls -la")

    assert spec["argv"] is None
    assert spec["shell_command"] == "ls -la"
    assert spec["env_extra"] == {}


def test_agent_shell_label_reflects_resolved_shell(monkeypatch):
    monkeypatch.setattr(platform_compat, "IS_WINDOWS", True)
    monkeypatch.setattr(platform_compat, "find_bash", lambda: r"C:\Program Files\Git\bin\bash.exe")
    assert platform_compat.agent_shell_label() == "Git Bash"

    monkeypatch.setattr(platform_compat, "find_bash", lambda: None)
    assert platform_compat.agent_shell_label() == "cmd.exe"

    monkeypatch.setattr(platform_compat, "IS_WINDOWS", False)
    assert platform_compat.agent_shell_label() == "bash"


def test_os_label_names_each_platform(monkeypatch):
    monkeypatch.setattr(platform_compat, "IS_WINDOWS", True)
    assert platform_compat.os_label() == "Windows"

    monkeypatch.setattr(platform_compat, "IS_WINDOWS", False)
    monkeypatch.setattr(platform_compat.platform, "system", lambda: "Darwin")
    assert platform_compat.os_label() == "macOS"

    monkeypatch.setattr(platform_compat.platform, "system", lambda: "Linux")
    assert platform_compat.os_label() == "Linux"
