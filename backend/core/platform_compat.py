...
# ── Agent command shell (the `bash` tool) ────────────────────────────────────
def os_label() -> str:
    """Friendly OS name for prompts/UI: 'Windows', 'macOS', or 'Linux'."""
    if IS_WINDOWS:
        return "Windows"
    sys_name = platform.system()
    return "macOS" if sys_name == "Darwin" else (sys_name or "Linux")


def agent_shell_label() -> str:
    """Name of the shell the agent's command tool actually runs through."""
    if IS_WINDOWS:
        return "Git Bash" if find_bash() else "cmd.exe"
    return "bash"


def agent_shell_invocation(command: str) -> dict:
    """Describe how to launch an agent shell command (the ``bash`` tool) on
    this OS. The two asyncio entry points need different argument shapes, so the
    caller branches on which key is populated:

      * ``argv`` set    → ``await asyncio.create_subprocess_exec(*argv, ...)``
      * ``shell_command`` set → ``await asyncio.create_subprocess_shell(cmd, ...)``

    Always merge ``env_extra`` into the child's environment.

    Windows with bash (Git Bash/WSL): run it as a **login** shell —
    ``[bash, "-lc", command]``. The ``-l`` is load-bearing, not cosmetic:
    ``shutil.which('bash')`` frequently resolves to ``…\\Git\\usr\\bin\\bash.exe``,
    and invoking that with a plain ``-c`` yields a shell with no ``/usr/bin`` on
    ``PATH`` — so ``ls``/``cat``/``grep`` fail with "command not found". A login
    shell sources ``/etc/profile`` (which repairs ``PATH``) regardless of which
    bash binary resolves. ``CHERE_INVOKING=1`` stops that profile from ``cd``-ing
    to ``$HOME``, so the agent's ``cwd`` is preserved.

    Windows without bash: ``cmd.exe`` via the shell. The agent is told in its
    prompt to use Windows command syntax instead of bash.

    POSIX: the system shell, unchanged.
    """
    bash = find_bash() if IS_WINDOWS else None
    if bash:
        return {
            "argv": [bash, "-lc", command],
            "shell_command": None,
            "env_extra": {"CHERE_INVOKING": "1"},
        }
    return {"argv": None, "shell_command": command, "env_extra": {}}
