...
class GalleryImage(TimestampMixin, Base):
    ...
    # Multimodal model output: distinguishes generated audio/video from images
    media_type     = Column(String, nullable=True, default="image")  # image|audio|video
    duration_secs  = Column(Integer, nullable=True)                  # audio/video length
...
class ModelEndpoint(TimestampMixin, Base):
    ...
    # Optional per-endpoint override of model output modalities (JSON list, e.g.
    # ["text","image"]). NULL = fall back to the model_media registry/heuristics.
    output_modalities = Column(Text, nullable=True, default=None)
...
def _migrate_add_output_modalities_column():
    """Add output_modalities column to model_endpoints if it doesn't exist."""
    import sqlite3
    db_path = DATABASE_URL.replace("sqlite:///", "")
    if not os.path.exists(db_path):
        return
    ...
    try:
        ...
        cursor = conn.execute("PRAGMA table_info(model_endpoints)")
        columns = [row[1] for row in cursor.fetchall()]
        if columns and "output_modalities" not in columns:
            conn.execute("ALTER TABLE model_endpoints ADD COLUMN output_modalities TEXT")
            conn.commit()
            logging.getLogger(__name__).info("Migrated: added 'output_modalities' column to model_endpoints")
    except Exception as e:
        logging.getLogger(__name__).warning(f"output_modalities migration failed: {e}")
    finally:
        try:
            conn.close()
        except Exception:
...
def _migrate_add_gallery_media_columns():
    """Add media_type/duration_secs columns to gallery_images if they don't exist
    (for model-emitted audio/video output)."""
    import sqlite3
    db_path = DATABASE_URL.replace("sqlite:///", "")
    if not os.path.exists(db_path):
        return
    ...
    try:
        conn = sqlite3.connect(db_path)
        cursor = conn.execute("PRAGMA table_info(gallery_images)")
        columns = [row[1] for row in cursor.fetchall()]
        if columns and "media_type" not in columns:
            conn.execute("ALTER TABLE gallery_images ADD COLUMN media_type TEXT DEFAULT 'image'")
            logging.getLogger(__name__).info("Migrated: added 'media_type' column to gallery_images")
        if columns and "duration_secs" not in columns:
            conn.execute("ALTER TABLE gallery_images ADD COLUMN duration_secs INTEGER")
            logging.getLogger(__name__).info("Migrated: added 'duration_secs' column to gallery_images")
        conn.commit()
    except Exception as e:
        logging.getLogger(__name__).warning(f"gallery media columns migration failed: {e}")
    finally:
        try:
            conn.close()
        except Exception:
...
def init_db():
    ...
    _migrate_add_output_modalities_column()
