...
def setup_auth_routes(auth_manager: AuthManager) -> APIRouter:
    ...
    async def first_run_setup(body: SetupRequest, request: Request):
        ...
        if not _setup_limiter.check(getattr(request.client, 'host', None) or 'unknown'):
    ...
    async def signup(body: SignupRequest, request: Request):
        ...
        if not _signup_limiter.check(getattr(request.client, 'host', None) or 'unknown'):
            raise HTTPException(429, "Too many requests — try again later")
    ...
    async def login(body: LoginRequest, request: Request, response: Response):
        if not _login_limiter.check(getattr(request.client, 'host', None) or 'unknown'):
            raise HTTPException(429, "Too many requests — try again later")
    ...
    async def set_settings(request: Request):
        ...
        _INT_RANGES = {
            ...
            "agent_max_rounds_ceiling": (1, 400),  # hard cap for auto-continue
            "agent_subagent_max_rounds": (1, 60),
        }
