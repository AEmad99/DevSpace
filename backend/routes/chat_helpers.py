...
def refresh_session_endpoint_url(sess, session_id: str, owner: Optional[str] = None) -> bool:
    """Re-derive the session's ``endpoint_url`` from its ModelEndpoint base.

    Covers the historical bug where the resolver built broken URLs for MiniMax
    (and any other provider whose surface path isn't a plain ``/chat/completions``
    suffix). A session that was created before the fix has a stale
    ``endpoint_url`` persisted in the DB; every subsequent chat uses it
    verbatim, so even after the resolver is fixed those sessions keep 404'ing
    until the URL is manually refreshed. This helper compares the saved URL
    against what ``build_chat_url(ep.base_url)`` would produce today and, if
    they differ, rewrites both the in-memory ``sess.endpoint_url`` and the
    DB row.

    Headers get rebuilt alongside the URL — when the URL surface changes
    (e.g. ``/chat/completions`` -> ``/anthropic/v1/messages``) the auth pair
    has to change too (``Authorization: Bearer`` -> ``x-api-key``). Doing
    both in one place keeps them consistent.
    """
    try:
        from src.endpoint_resolver import build_chat_url, build_headers, resolve_endpoint_runtime
        target_url = (getattr(sess, "endpoint_url", "") or "").rstrip("/")
        if not target_url:
            return False
        db = SessionLocal()
        try:
            ...
            for ep in q.all():
                ...
                try:
                    ...
                    continue
                new_url = build_chat_url(base)
                if not new_url or new_url.rstrip("/") == target_url:
                    return False
                # URL drifted — rewrite it (and rebuild headers in case the
                # surface-specific auth pair changed too).
                new_headers = build_headers(api_key, base) if api_key else sess.headers
                ...
                db.commit()
                logger.info(
                    "Refreshed stale endpoint_url for session %s: %s -> %s (endpoint %s)",
                    session_id, target_url, new_url, getattr(ep, "name", "?"),
                )
                return True
            return False
        finally:
        ...
        logger.warning(f"Failed to refresh session endpoint_url for {session_id}: {e}")
        return False
...
def _generated_image_to_data_url(ref):
    """Read a stored generated image → base64 data URL (or None). Generated files
    live on local disk; the model can't fetch the /api/generated-image URL, so we
    inline them as data URLs."""
    try:
        import base64
        from pathlib import Path
        from src.constants import GENERATED_IMAGES_DIR
        fn = (ref.get("url") or "").rsplit("/", 1)[-1]
        if not fn:
            return None
        p = Path(GENERATED_IMAGES_DIR) / fn
        if not p.exists():
            return None
...
def inject_generated_media_context(messages, sess, model, max_images=2):
    """Feed recently GENERATED images back into context as vision input so a
    vision-capable model can see/judge what it produced — unified context across
    media. No-op for text-only models or when nothing was generated."""
    ...
    for msg in reversed(getattr(sess, "history", None) or []):
        ...
        for mo in (md.get("model_outputs") or []):
            if mo.get("media") == "image" and mo.get("url"):
                refs.append(mo)
    ...
    if not refs:
        return messages
    refs.reverse()  # chronological
    parts = [{"type": "text", "text": "For reference, here are the image(s) you generated earlier in this conversation:"}]
    for ref in refs:
        du = _generated_image_to_data_url(ref)
        if du:
            parts.append({"type": "image_url", "image_url": {"url": du}})
    if len(parts) == 1:
        return messages  # files missing
    media_msg = {"role": "user", "content": parts}
    insert_at = len(messages)
    for i in range(len(messages) - 1, -1, -1):
        if messages[i].get("role") == "user":
            insert_at = i
            break
    return messages[:insert_at] + [media_msg] + messages[insert_at:]
...
async def build_chat_context(
    ...
    # Feed recently generated images back to vision-capable models so the model
    # can actually see/judge what it produced (added after trim so the images
    # aren't dropped). Keeps conversation context unified across media.
    messages = inject_generated_media_context(messages, sess, sess.model)
...
def save_assistant_response(
    ...
    *,
    ...
    if model_outputs:
        md["model_outputs"] = model_outputs  # native model-emitted media (url/id refs)
