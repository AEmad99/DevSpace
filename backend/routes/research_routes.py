...
# --- Visual-report open-in-external-browser auth ---------------------------
# The visual report is opened in the user's EXTERNAL default browser (via the
# Tauri opener plugin), which carries none of the in-app WebView's session
# cookie -- so a plain GET /api/research/report/{id} would 401 with
# "Not authenticated". To bridge that, the (cookie-authed) WebView first mints a
# short-lived, session+owner-scoped token via POST /api/research/report-link/{id}
# and opens /api/research/report/{id}?t=<token>. That GET is auth-exempt at the
# middleware layer (see app.py AUTH_EXEMPT_PATTERNS) and proves identity from the
# token itself -- the same "path/query is the credential" pattern used for task
# webhook tokens. On a successful token-authed load the report response also sets
# a normal session cookie so the report page's own same-origin interactive
# fetches (hide-image, unhide-images, spinoff) work in that browser.
_REPORT_TOKEN_TTL = 600  # seconds (10 min) -- enough to open + interact briefly
_report_tokens: dict = {}  # token -> (session_id, owner, expiry_epoch)


def _prune_report_tokens(now: float) -> None:
    for k in [k for k, (_, _, exp) in _report_tokens.items() if now > exp]:
        _report_tokens.pop(k, None)


def _mint_report_token(session_id: str, owner: str) -> str:
    now = time.time()
    _prune_report_tokens(now)
    tok = secrets.token_urlsafe(32)
    _report_tokens[tok] = (session_id, owner or "", now + _REPORT_TOKEN_TTL)
    return tok


def _resolve_report_token(token: Optional[str], session_id: str):
    """Return a 1-tuple ``(owner,)`` for a live token bound to ``session_id``,
    else ``None``. Wrapped in a tuple because ``owner`` is legitimately ``""``
    in single-user mode."""
    if not token:
        return None
    rec = _report_tokens.get(token)
    if not rec:
        return None
    sid, owner, exp = rec
    if time.time() > exp:
        ...
        return None
    if not secrets.compare_digest(sid, session_id):
        return None
    return (owner,)


def _authorize_report_request(request: Request, session_id: str, token: Optional[str]):
    """Resolve the user for a (possibly cookie-less) visual-report GET.

    Returns ``(user, cookie_user)`` where ``cookie_user`` is non-None only when
    the request authenticated via a report token -- the caller then issues a
    fresh session cookie so the external browser can make the report's
    same-origin interactive calls. Raises 401 when it can't authenticate."""
    # 1) Short-lived report token (external browser, no session cookie).
    rec = _resolve_report_token(token, session_id)
    if rec is not None:
        owner = rec[0]
        return owner, owner
    # 2) Normal session. Middleware is skipped for this exempt path, so trust
    #    request.state if set, otherwise validate the cookie ourselves.
    ...
    if auth_mgr is not None and getattr(auth_mgr, "is_configured", False):
        ...
        if cookie and auth_mgr.validate_token(cookie):
            return auth_mgr.get_username_for_token(cookie), None
        raise HTTPException(401, "Not authenticated")
    # 3) Auth disabled / unconfigured first-run: allow (loopback) single-user.
    if _auth_disabled():
        return "", None
...
def _grant_report_session_cookie(response, request: Request, user: str) -> None:
    """Issue a normal session cookie on a token-authed report response so the
    external browser can make the report's same-origin interactive calls."""
    if not user:
        return
    app = getattr(request, "app", None)
    auth_mgr = getattr(getattr(app, "state", None), "auth_manager", None)
    if auth_mgr is None or not getattr(auth_mgr, "is_configured", False):
        return
    try:
        token = auth_mgr.create_session_trusted(user)
    except Exception:
        token = None
    if not token:
        return
...
def setup_research_routes(research_handler, session_manager=None) -> APIRouter:
    ...
    @router.post("/api/research/report-link/{session_id}")
    async def research_report_link(session_id: str, request: Request):
        """Mint a short-lived token to open this report in an external browser.
        Cookie-authed (called from the in-app WebView); the returned URL embeds
        a token the auth-exempt report GET can verify on its own."""
        user = _require_user(request)
        _validate_session_id(session_id)
        _assert_owns_research(session_id, user)
        token = _mint_report_token(session_id, user)
        return {"token": token, "url": f"/api/research/report/{session_id}?t={token}"}

    @router.get("/api/research/report/{session_id}")
    async def research_report(session_id: str, request: Request, t: Optional[str] = Query(None)):
        """Serve the visual HTML report for a completed research session.

        Auth-exempt at the middleware layer so it can open in an EXTERNAL
        browser with no session cookie; it authenticates itself here via a
        report token (``?t=``) or a normal session cookie. See the module-level
        notes on the report-token bridge."""
        _validate_session_id(session_id)
        user, cookie_user = _authorize_report_request(request, session_id, t)
        _assert_owns_research(session_id, user)
        ...
        if cookie_user is not None:
            _grant_report_session_cookie(response, request, cookie_user)
        return response
    ...
    class ResearchStartRequest(BaseModel):
        ...
        # M2: optional knowledge source (issue #2). Format:
        #   {"type": "folder", "config": {"path": "/abs/path", "extensions": [...]}}
        # When omitted (or feature flag is off) the default InternetSource is used.
        source: Optional[Dict] = None
        # M4: optional LIST of sources for hybrid mode. The picker emits
        # this when the user added more than one row. When both `source`
        # and `sources` are present, `sources` wins.
    ...
    async def research_start(body: ResearchStartRequest, request: Request):
        ...
        # M2: resolve the optional `source` config into a Source instance.
        # When omitted (or feature flag off), sources=None → DeepResearcher
        # builds [InternetSource(delegate=self)] automatically, preserving
        # the legacy behavior. When a non-internet source is requested but
        # the flag is off, we reject with a clear error rather than silently
        # ignoring it.
        sources = None
        try:
            from src.constants import RESEARCH_SOURCES_ENABLED
        except Exception:
            RESEARCH_SOURCES_ENABLED = False

        # M4: the UI may send `sources` (plural, list) for hybrid mode.
        # Prefer that when present.
        spec_list = body.sources if body.sources else ([body.source] if body.source else None)
        if spec_list:
            if not RESEARCH_SOURCES_ENABLED:
                raise HTTPException(
                    400,
                    "Custom research sources are disabled. Set "
                    "RESEARCH_SOURCES_ENABLED=true to enable folder / codebase / "
            ...
            try:
                ...
                for spec in spec_list:
                    if not isinstance(spec, dict):
                        raise HTTPException(400, "Each source spec must be an object")
                    src_type = spec.get("type")
                    if not src_type:
                        raise HTTPException(400, "source.type is required")
                    src_config = dict(spec.get("config") or {})
                    # User-scoped sources (Library, Previous chats, Documents)
                    # need the current user's identity to filter their data.
                    # Inject `owner` here so the Source classes stay
                    # route-agnostic and can be unit-tested without an HTTP
                    # request.
                    if src_type in ("library", "chats", "documents"):
                        src_config.setdefault("owner", user or "")
                    sources.append(registry.get(src_type, src_config))
            except HTTPException:
                raise
            except KeyError as e:
                raise HTTPException(400, f"Unknown source type: {e}")
            except Exception as e:
                logger.warning(f"Failed to instantiate source: {e}", exc_info=True)
                raise HTTPException(400, f"Failed to initialize source: {e}")
