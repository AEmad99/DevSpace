...
def _is_discovery_only_provider(provider: str) -> bool:
    return provider in {"chatgpt-subscription", "grok-subscription"}
...
def _probe_endpoint(base_url: str, api_key: str = None, timeout: int = 5) -> List[str]:
    ...
    if provider == "grok-subscription":
        from src.grok_subscription import fetch_available_models
        if api_key:
            return fetch_available_models(api_key, timeout=timeout)
        return []
    if provider == "anthropic":
    ...
    def _fetch_models(owner: str = "", is_admin: bool = False):
        ...
        for ep in endpoints:
            ...
            if model_ids:
                ...
                # MiniMax's dedicated media-generation models (image-01, Hailuo
                # video, speech-02) aren't returned by the chat /models list, so
                # surface them here for the direct api.minimax.io endpoint to make
                # them selectable. They route via the chat_stream media branch.
                if "minimax.io" in (base or ""):
                    for _mm in ("image-01", "MiniMax-Hailuo-02", "speech-02-hd"):
                        if _mm not in curated and _mm not in extra:
                            curated.append(_mm)
                from src.model_media import get_output_modalities as _get_out_mods
                out_mods = {mid: _get_out_mods(provider, mid, ep) for mid in (curated + extra)}
                items.append({
                    "host": "custom",
                    "port": 0,
                    "url": chat_url,
    ...
    async def toggle_model_endpoint(ep_id: str, request: Request):
        ...
        try:
            ...
            if body:
                ...
                if "output_modalities" in body:
                    _om = body.get("output_modalities")
                    if _om in (None, "", [], "auto"):
                        ep.output_modalities = None          # fall back to registry/heuristics
                    elif isinstance(_om, list):
                        _vals = [str(x).strip() for x in _om if str(x).strip()]
                        ep.output_modalities = json.dumps(_vals) if _vals else None
                    elif isinstance(_om, str):
                        _vals = [p.strip() for p in _om.split(",") if p.strip()]
                        ep.output_modalities = json.dumps(_vals) if _vals else None
            ...
            _local_probe_cache["data"] = None
            try:
                _om_out = json.loads(ep.output_modalities) if getattr(ep, "output_modalities", None) else None
            except Exception:
                _om_out = None
            return {
                "id": ep.id,
