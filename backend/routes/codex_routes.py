...
def _codex_ws_root() -> str:
    """The server-persisted Code Workspace root, re-vetted; 409 if unset/unusable.
    Same confinement primitives the in-app workspace routes use (Phase 1)."""
    from src.settings import get_setting
    from src.tool_execution import vet_workspace
    raw = (get_setting("code_workspace_root") or "").strip()
    resolved = vet_workspace(raw) if raw else None
    if not resolved:
        raise HTTPException(409, "No code workspace root is set (choose one in the Code Workspace panel).")
    return resolved


def _codex_confine(root: str, path: str) -> str:
    from src.tool_execution import _resolve_tool_path_in_workspace
    try:
        return _resolve_tool_path_in_workspace(root, path)
...
def setup_codex_routes(
    ...
    # ── Files (token-scoped external file access, confined to the workspace) ──
    # Mirrors the in-app /api/workspace file routes but gated by API-token
    # scopes (files:read / files:write) for the Codex bridge instead of owner
    # auth. Every path is confined to code_workspace_root via the same helper.
    import os as _os

    @router.get("/files/list")
    def codex_files_list(request: Request, path: str = ""):
        _scope_owner(request, FILES_READ_SCOPES)
        from src.agent_tools.filesystem_tools import _CODENAV_SKIP_DIRS
        root = _codex_ws_root()
        target = root if not path.strip() else _codex_confine(root, path)
        if not _os.path.isdir(target):
            raise HTTPException(400, f"Not a directory: {path or root}")
        entries = []
        try:
            with _os.scandir(target) as it:
                for e in it:
                    ...
                    try:
                        ...
                        if is_dir and e.name in _CODENAV_SKIP_DIRS:
                            continue
                        size = 0 if is_dir else e.stat(follow_symlinks=False).st_size
                    except OSError:
                        continue
                    entries.append({"name": e.name, "path": _os.path.join(target, e.name), "is_dir": is_dir, "size": size})
        except (PermissionError, OSError) as exc:
            raise HTTPException(403, f"Cannot read directory: {exc}")
        entries.sort(key=lambda x: (not x["is_dir"], x["name"].lower()))
        return {"root": root, "path": target, "entries": entries[:1000]}

    @router.get("/files/read")
    def codex_files_read(request: Request, path: str):
        _scope_owner(request, FILES_READ_SCOPES)
        from src.constants import MAX_READ_CHARS
        root = _codex_ws_root()
        full = _codex_confine(root, path)
        ...
        return {"path": full, "content": content, "truncated": truncated,
                "language": _os.path.splitext(full)[1].lstrip(".").lower()}

    @router.post("/files/write")
    def codex_files_write(request: Request, body: dict[str, Any] = Body(default_factory=dict)):
        _scope_owner(request, FILES_WRITE_SCOPES)
        root = _codex_ws_root()
        ...
        if not isinstance(content, str):
            raise HTTPException(400, "content (string) is required")
        full = _codex_confine(root, path)
        try:
            ...
            raise HTTPException(500, str(exc))
        return {"ok": True, "path": full, "size": len(content)}
