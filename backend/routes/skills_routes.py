...
class SkillGenerateRequest(BaseModel):
    """Body for POST /api/skills/generate — ask the configured LLM to author
    a SKILL.md from a free-form description and save it on disk."""
    description: str = Field(..., min_length=4, max_length=4000)
    category: str = Field("general", max_length=40)
    # Optional hint to nudge the LLM (e.g. "command-line tool", "email triage").
    # Kept short so a single user prompt can fit comfortably under 8K tokens.
    hints: Optional[str] = Field(None, max_length=800)
...
def _resolve_generate_models(owner=None):
    """Resolve (url, model, headers) for the /api/skills/generate call.

    Mirrors _resolve_audit_models: try Utility first (the model the user has
    configured for background tasks like summarization/audit), then fall back
    to the Default chat model. Normalizes against the served model list so a
    stale configured id doesn't 404 the generator. Raises ValueError if no
    model is configured at all — the caller turns that into a 400.
    """
...
_GENERATE_SYS_PROMPT = """\
You are authoring a reusable AI "skill" — a saved procedure the agent will
follow on future requests that match its trigger. Output ONE single complete
SKILL.md document, nothing else (no preamble, no commentary, no code fences
wrapping the whole document).

Format (frontmatter is REQUIRED, body sections are strongly recommended):

---
name: kebab-case-slug                       # short, descriptive, max ~60 chars
description: one-line summary               # surfaces in the catalog
version: 1.0.0
category: <single-word category>            # e.g. dev, writing, email, data
tags: [comma-separated, lowercase]          # 2-5 narrow retrieval tags
status: draft
confidence: 0.7
source: learned
---
...
## When to Use
Describe the trigger — what user request or situation should activate this
skill. Be specific so the skill doesn't over-fire on adjacent requests.

## Procedure
1. First concrete step the agent should take.
2. Next step ...
3. ...
...
## Verification
- How the agent should confirm the procedure actually worked.

Rules:
- The `name` MUST be a kebab-case slug (lowercase, dashes, no spaces).
- The procedure must be concrete and reproducible — not generic advice.
- Use real tools the agent has, not invented ones. Don't reference commands
  that aren't standard on the platform.
- Keep the procedure short enough to read at a glance (roughly <= 8 steps).
- Retrieval tags and `when_to_use` must be NARROW — broad tags like "system",
  "general", "tool" cause the skill to over-fire and should be avoided.
- Output ONLY the SKILL.md text. Do NOT wrap it in ``` fences or add any
  prose before/after it.
"""


def _extract_skill_md(raw: str) -> Optional[str]:
    """Pull the SKILL.md text out of an LLM response.

    The model is told to reply with raw SKILL.md text, but we still tolerate
    a layer of leading/trailing prose (especially from reasoning models that
    think before they answer): strip  think  blocks, drop an outer
    ```markdown fence if present, then keep only the last well-formed
    `--- ... ---` frontmatter document so reviewer prose never leaks into
    the saved file. Trailing prose after the last ``## `` heading is
    discarded for the same reason.
    """
    import re as _re
    if not raw:
        return None
    text = _re.sub(r"<think(?:ing)?>[\s\S]*?</think(?:ing)?>", "", raw, flags=_re.I)
    text = _re.sub(r"<think(?:ing)?>[\s\S]*$", "", text, flags=_re.I).strip()
    # Strip an outer ```markdown / ``` fence if the model wrapped the whole
    # document. Keep only the markdown in between.
    fence = _re.match(r"^```(?:markdown|md)?\s*\n([\s\S]*?)\n```\s*$", text)
    if fence:
        text = fence.group(1).strip()
    # Keep only the last complete-looking frontmatter document — the
    # reasoning-trace that some models prepend before the final answer must
    # not leak into the saved file.
    starts = list(_re.finditer(r"(?m)^---\s*\n(?=[\s\S]*?^name\s*:)", text))
    if not starts:
        return None
    doc = text[starts[-1].start():]
    # Trim trailing prose after the last ``## `` heading. The body of a
    # SKILL.md is well-formed if it ends right after a section's last
    # paragraph; anything past it is conversational filler like "Let me
    # know if you'd like changes".
    last_heading = None
    for m in _re.finditer(r"(?m)^##\s+", doc):
        last_heading = m
    if last_heading is not None:
        tail = doc[last_heading.start():]
        # Drop everything after the last blank line in the tail (the section
        # content stops at the first blank line after the last paragraph).
        m = list(_re.finditer(r"\n\s*\n", tail))
        if m:
            doc = doc[: last_heading.start() + m[-1].end()].rstrip() + "\n"
    return doc.strip() or None


async def _llm_generate_skill_md(description: str, category: str,
                                  hints: Optional[str],
                                  url: str, model: str,
                                  headers: Optional[dict]) -> str:
    """Call the configured LLM and return the SKILL.md text it produced.

    Two attempts, same defensive pattern used elsewhere in this module for
    LLM-as-judge calls: the first lets a heavy reasoning model think; if it
    never emits the document (budget burned inside  think ), the second
    forbids thinking and demands the markdown directly.
    """
    from src.llm_core import llm_call_async
    user_msg = (
        f"Write a SKILL.md that captures the following reusable procedure.\n\n"
        f"Description: {description.strip()}\n"
        f"Category: {(category or 'general').strip() or 'general'}\n"
    )
    if hints and hints.strip():
        user_msg += f"Hints: {hints.strip()}\n"
    user_msg += "\nRemember: emit ONLY the SKILL.md document."

    last_err: Optional[Exception] = None
    for attempt in range(2):
        sys_content = _GENERATE_SYS_PROMPT
        if attempt == 1:
            sys_content += (
                "\n\nDo NOT use  think  or any reasoning. Your reply must START "
                "with `---` (the YAML frontmatter opener) and contain ONLY the "
                "SKILL.md text."
            )
        try:
            raw = await llm_call_async(
                ...
                [{"role": "system", "content": sys_content},
                 {"role": "user", "content": user_msg}],
                # Generous budget so a reasoning model can think AND still
                # have room for the ~1-2K-token SKILL.md it needs to emit.
                temperature=0.4, max_tokens=8192, headers=headers, timeout=180,
            )
        except Exception as e:
            last_err = e
            continue
        md = _extract_skill_md(raw)
        if md:
            return md
    ...
    raise HTTPException(
        502,
        "The model did not return a recognizable SKILL.md. Try a different "
        "model, or refine the description.",
    )
...
def setup_skills_routes(skills_manager: SkillsManager) -> APIRouter:
    ...
    async def generate_skill(request: Request, body: SkillGenerateRequest):
        """Ask the configured LLM to author a SKILL.md from a free-form
        description, then save it to disk so it shows up in /api/skills and
        is invokable as `/<skill-name>` immediately.

        Uses source='user' (skips auto-dedup) and status='published' (visible
        in the slash catalog) so the user can try the skill right away in the
        current chat or any other. The audit pipeline still picks the skill
        up and can demote it on a subsequent run if it turns out to be broken
        or generic.
        """
        ...
        user = _owner(request)
        try:
            url, model, headers = _resolve_generate_models(owner=user)
        except ValueError as e:
            raise HTTPException(400, str(e)) from e

        try:
            md = await _llm_generate_skill_md(
                body.description, body.category, body.hints,
                url, model, headers,
            )
        except HTTPException:
            raise
        except Exception as e:
            logger.warning("skill-generate: LLM call failed: %s", e)
            raise HTTPException(502, f"LLM call failed: {e}") from e

        # Parse to validate frontmatter + body. Pin the name on disk to the
        # slug the manager derives from the LLM's chosen `name` (or the
        # description if the LLM forgot) so the path always matches what the
        # caller will see in the response.
        try:
            sk = _Skill.from_markdown(md)
        except Exception as e:
            logger.warning("skill-generate: could not parse model output: %s", e)
            raise HTTPException(
                502,
                f"The model returned something that isn't a valid SKILL.md: {e}",
            ) from e

        # Write the parsed Skill back out (frontmatter normalized) before
        # handing to add_skill so the on-disk file matches the parser output.
        normalized_md = sk.to_markdown()
        try:
            entry = skills_manager.add_skill(
                name=sk.name,
                description=sk.description or body.description,
                category=(body.category or sk.category or "general"),
                tags=list(sk.tags or []),
                platforms=list(sk.platforms or []),
                requires_toolsets=list(sk.requires_toolsets or []),
                fallback_for_toolsets=list(sk.fallback_for_toolsets or []),
                when_to_use=sk.when_to_use,
                procedure=list(sk.procedure or []),
                pitfalls=list(sk.pitfalls or []),
                verification=list(sk.verification or []),
                status="published",
                version=sk.version or "1.0.0",
                confidence=float(sk.confidence or 0.7),
                # source='user' exempts the skill from Jaccard auto-dedup —
                # a user explicitly asked for it.
                source="user",
                teacher_model=model,
                session_id=None,
                owner=user,
            ...
            logger.error("skill-generate: add_skill failed: %s", e)
            raise HTTPException(500, f"Saving the skill failed: {e}") from e

        if not entry.get("_deduped"):
            _fire_skill_added(user)
        # Surface what was actually saved — the model's parsed name, the
        # normalized frontmatter, and the body it produced — so the caller
        # can show the user a preview and offer to edit before they invoke it.
        return {
            "ok": True,
            "skill": entry,
            "markdown": normalized_md,
            "model": model,
            "command": f"/{entry.get('name')}",
        }
