...
def setup_chat_routes(
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        # Native multimodal OUTPUT: opt-in audio reply ("speak") + voice/format.
        # (Image output is auto-requested for image-capable models, no toggle.)
        want_media = str(form_data.get("want_media") or (body or {}).get("want_media") or "").lower() == "true"
        audio_voice = form_data.get("audio_voice") or (body or {}).get("audio_voice")
        audio_format = form_data.get("audio_format") or (body or {}).get("audio_format")
        # Smart media routing: auto-detect image/audio/video generation requests
        # in chat mode (default on). force_media = a confirmed video (or explicit
        # modality) that bypasses the classifier.
        smart_media = str(form_data.get("smart_media") if form_data.get("smart_media") is not None else "true").lower() != "false"
        force_media = (form_data.get("force_media") or "").strip().lower()
        media_prompt = form_data.get("media_prompt") or ""
        ...
        # Self-heal: rewrite a stale ``endpoint_url`` that was saved before a
        # provider added a new surface (MiniMax's Anthropic-compat was missing
        # from the resolver pre-fix; sessions saved back then still carry the
        # broken /chat/completions URL and would 404 on every send).
        refresh_session_endpoint_url(sess, session, owner=effective_user(request))
        ...
        async def stream_with_save() -> AsyncGenerator[str, None]:
            ...
            full_response = ""
            last_metrics = None
            _model_outputs = []  # native model-emitted media refs (image/audio/video)
            from src.model_media import media_gen_kind as _media_gen_kind
            _media_kind = _media_gen_kind(sess.model)  # dedicated image/audio/video gen model?
            ...
            # ── Smart media routing (applies to BOTH chat and agent mode) ──────
            # Stay on the chat model (e.g. MiniMax-M3); if the message asks to
            # GENERATE media, the LLM evaluates it and we auto-route to the right
            # media model — no manual model switch. The media endpoint
            # (api.minimax.io) is resolved independently of which endpoint the chat
            # model runs on. Skipped when a dedicated media model is already
            # selected (_media_kind) or it's a gpt-image/DALL-E session.
            _img_gen_sess = _is_image_generation_session(sess, owner=_user)
            _route_kind, _route_prompt = None, None
            _route_voice, _route_emotion, _route_vm = None, None, None
            if not _img_gen_sess and not _media_kind:
                from src.model_media import (
                    maybe_media_request as _mm_gate,
                    classify_media_intent as _mm_classify,
                    parse_voice_directives as _mm_parse_voice,
                    logger as _mm_logger,
                )
                if force_media in ("image", "audio", "music", "video"):
                    _route_kind = force_media
                    _route_prompt = (media_prompt or message or "").strip()
                elif smart_media and _mm_gate(message or ""):
                    _route_kind, _route_prompt, _route_voice, _route_emotion, _route_vm = await _mm_classify(
                        message or "", sess.endpoint_url, sess.model, sess.headers)
                # Regex fallback: the LLM classifier sometimes drops voice
                # characteristics (or the user message is short and obvious).
                # Parse voice descriptors from the ORIGINAL user message — if
                # the LLM already set a field, keep its value; only fill in
                # what was missed. This is what makes "generate a man's voice
                # saying X" reliably pick a male voice even when the classifier
                # returns audio + X verbatim.
                if _route_kind == "audio":
                    _rgx_clean, _rgx_voice, _rgx_emotion, _rgx_vm = _mm_parse_voice(
                        message or "")
                    if _rgx_voice and not _route_voice:
                        _route_voice = _rgx_voice
                    if _rgx_emotion and not _route_emotion:
                        _route_emotion = _rgx_emotion
                    if _rgx_vm and not _route_vm:
                        _route_vm = _rgx_vm
                    # The regex is the source of truth for the SPOKEN TEXT.
                    # The LLM classifier is non-deterministic — it sometimes
                    # returns the raw user message verbatim and sometimes a
                    # partial clean. Either way, what reaches the T2A model
                    # must be just the words the user wanted spoken, not the
                    # meta-instruction wrap. We log when the LLM and the
                    # regex disagree so future regressions are observable.
                    if _rgx_clean and (not _route_prompt
                                        or _route_prompt.strip() != _rgx_clean.strip()):
                        _mm_logger.info(
                            "audio prompt sanitised: llm=%r -> regex=%r",
                            (_route_prompt or "")[:200], _rgx_clean[:200])
                    if _rgx_clean:
                        _route_prompt = _rgx_clean
            ...
            if _img_gen_sess:
                ...
                # ── Dedicated media-generation model (image-01 / speech-02 /
                # Hailuo): call the provider's media API inline over this SSE
                # stream, reusing model_media storage + the frontend's
                # model_media rendering. Video is long-running (submit→poll) and
                # survives navigation via the background-stream path.
                from src.model_media import generate_media_events, parse_voice_directives
                _mk_text = message or ""
                _mk_voice = _mk_emotion = _mk_vm = None
                if _media_kind == "audio":
                    _mk_text, _mk_voice, _mk_emotion, _mk_vm = parse_voice_directives(_mk_text)
                async for _vchunk in generate_media_events(
                    _media_kind, sess.endpoint_url, sess.headers, sess.model, _mk_text,
                    voice=_mk_voice, emotion=_mk_emotion, voice_modify=_mk_vm,
                    session_id=session, owner=_user,
                ):
                    if _vchunk.startswith("data: ") and not _vchunk.startswith("data: [DONE]"):
                        try:
                            _vd = json.loads(_vchunk[6:])
                            if _vd.get("type") == "model_media":
                                _model_outputs.append({k: _vd[k] for k in
                                    ("media", "url", "id", "mime", "duration_secs") if k in _vd})
                            elif _vd.get("delta"):
                                full_response += _vd["delta"]
                        except json.JSONDecodeError:
                            pass
                    yield _vchunk
                # Emit a complete caption (also the context record) instead of
                # leaving the live "Generating…" status as the message text.
                if _model_outputs:
                    from src.model_media import media_record as _mrec
                    full_response = _mrec(_media_kind, message or "", sess.model)
                    yield f'data: {json.dumps({"delta": full_response})}\n\n'
                if (full_response or _model_outputs) and not incognito:
                    save_assistant_response(
                        sess, session_manager, session,
                        full_response or "Generated media.", None,
                        model_outputs=_model_outputs or None, incognito=incognito)
                yield f'data: {json.dumps({"type": "metrics", "data": {"total_time": 0}})}\n\n'
                yield "data: [DONE]\n\n"
                _active_streams.pop(session, None)
                return
            elif _route_kind:
                # ── Auto-routed media generation (image/audio/video) ──────────
                # Works in chat AND agent mode: the user stayed on a chat model
                # and asked for media; generate it on the resolved media model and
                # persist a textual record for unified context.
                from src.model_media import (
                    resolve_media_endpoint, generate_media_events, media_record,
                    MINIMAX_MEDIA_FOR_KIND,
                )
                if "minimax.io" in (sess.endpoint_url or ""):
                    _mbase, _mheaders = sess.endpoint_url, sess.headers
                    _mmodel = MINIMAX_MEDIA_FOR_KIND[_route_kind]
                else:
                    _res = resolve_media_endpoint(_route_kind, owner=_user)
                    _mbase, _mheaders, _mmodel = _res if _res else (None, None, None)
                if not _mbase:
                    yield f'data: {json.dumps({"delta": f"I can generate {_route_kind}, but no media-capable endpoint is configured. Add a MiniMax (api.minimax.io) endpoint to enable it."})}\n\n'
                    yield "data: [DONE]\n\n"
                    _active_streams.pop(session, None)
                    return
                # Video is slow + costs more → confirm first (unless already
                ...
                if _route_kind == "video" and force_media != "video":
                    ...
                    _active_streams.pop(session, None)
                    return
                yield f'data: {json.dumps({"type": "model_info", "model": _mmodel})}\n\n'
                async for _ch in generate_media_events(
                    _route_kind, _mbase, _mheaders, _mmodel, _route_prompt,
                    voice=_route_voice, emotion=_route_emotion, voice_modify=_route_vm,
                    session_id=session, owner=_user,
                ):
                    if _ch.startswith("data: ") and not _ch.startswith("data: [DONE]"):
                        try:
                            _d = json.loads(_ch[6:])
                            if _d.get("type") == "model_media":
                                _model_outputs.append({k: _d[k] for k in
                                    ("media", "url", "id", "mime", "duration_secs") if k in _d})
                            elif _d.get("delta"):
                                full_response += _d["delta"]
                        except json.JSONDecodeError:
                            pass
                    yield _ch
                # Emit a complete caption (also the unified-context record) instead
                # of leaving the live "Generating…" status as the message text.
                _cap = media_record(_route_kind, _route_prompt, _mmodel)
                if _model_outputs:
                    full_response = _cap
                    yield f'data: {json.dumps({"delta": _cap})}\n\n'
                if (_model_outputs or full_response) and not incognito:
                    save_assistant_response(
                        sess, session_manager, session,
                        full_response or _cap, None,
                        model_outputs=_model_outputs or None, incognito=incognito)
                yield f'data: {json.dumps({"type": "metrics", "data": {"total_time": 0}})}\n\n'
                yield "data: [DONE]\n\n"
                _active_streams.pop(session, None)
                        ...
                        if chunk.startswith("data: ") and not chunk.startswith("data: [DONE]"):
                            try:
                                ...
                                if "delta" in data:
                                    ...
                                    _model_outputs.append({k: data[k] for k in
                                        ("media", "url", "id", "mime", "transcript", "duration_secs", "index")
                                        if k in data})
                                    yield chunk
                            ...
                            if full_response:
                                _saved_id = save_assistant_response(
                                    ...
                                    model_outputs=_model_outputs or None,
                                    incognito=incognito,
                                )
                                if _saved_id:
                        ...
                        if chunk.startswith("data: ") and not chunk.startswith("data: [DONE]"):
                            try:
                                ...
                                if "delta" in data:
                                    ...
                                    "todo_update",
                                    "edit_pending",
                                    "subagent_start", "subagent_step", "subagent_done",
                                ):
